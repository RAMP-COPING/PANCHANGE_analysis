...
Create dem numeric variables
```{r create dem numeric variables}
exclude_cols <- c("ID",
                  "Sample",
                  "startDate.glad",
                  "Age_uncleaned",
...
glad.dem.raw.id.numeric <- add_numeric(glad.dem.raw.id, exclude = exclude_cols)

# Check if factor columns have been added
colnames(glad.dem.raw.id.numeric)

# Dimensions before adding factor columns
dim(glad.dem.raw.id)
# Dimensions after adding factor columns
dim(glad.dem.raw.id.numeric)
```
...
length(
  which(
  glad.dem.raw.id.numeric$Age_uncleaned > age_upper_limit |
    glad.dem.raw.id.numeric$Age_uncleaned < age_lower_limit
     )
)

# remove age outliers
glad.dem.raw.id.numeric <- glad.dem.raw.id.numeric %>%
  mutate(
    Age = 
      if_else(
