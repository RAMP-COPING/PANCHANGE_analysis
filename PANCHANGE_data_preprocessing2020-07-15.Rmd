...
+++CH04-07-20: I have added all read-ins of the data. How will the coping data be stored? -> Question for Yuhao
+++CH04-07-20: We need to discuss the naming of the variables PHQ9 vs phq
+++CH04-07-20: We need to discuss the naming of the variables .0 vs .retro

GLAD only receives current measures and RAMP questionnaire

All COPING surveys are separate surveys
+++TOPHER
***
++KT: Need to make sure that the startDate's for each survey are coded accordingly, e.g. startDate.glad, startDate.coping, startDate.RAMP. Otherwise people who are in both GLAD and COPING will have two dates.
+++CH: I have added this

## GLAD demographics
# Read in data
```{r glad dem cols}
glad.dem.raw <- readRDS(file = "../data_raw/glad/dem_glad.rds")
dim(glad.dem.raw)
...
```
++KT: The age variable here needs to be cleaned - have added a chunk above (before age grouping) for this. Before: min = -55 and max = 290. 
```{r glad dem summary}
...
```{r glad gad cols}
glad.gad.raw <- readRDS(file = "../data_raw/glad/gad_glad.rds")
dim(glad.gad.raw)
...
```{r glad mhq cols}
glad.mhq.raw <- readRDS(file = "../data_raw/glad/mhq_glad.rds")
dim(glad.mhq.raw)
...
1. Depressive disorder group: major depression, antenatal/postnatal depression, premenstrual depression.
...
Depressive disorders group
+++CH: NEEDS BETTER LOGIC FOR MALES!!!!
++KT: Sex issue fixed - code has been checked. 407 people with no MDD but have either postnatal/pmdd.
...
```
+++CH: Why are there so many NAs in mhq.panic_disorder?
++KT: Added late to questionnaire. 
...
+++CH: Lots of NAs in schizoaffective_disorder 
...
```{r glad pcl cols}
glad.pcl.raw <- readRDS(file = "../data_raw/glad/pcl_glad.rds")
dim(glad.pcl.raw)
...
```{r glad phq cols}
glad.phq.raw <- readRDS(file = "../data_raw/glad/phq_glad.rds")
dim(glad.phq.raw)
...
coping.glad.gad.raw <- readRDS(file = "../data_raw/coping_glad/gad_coping_glad.rds")
...
+++ ADD 3 CHUNKs for phq, gad for each comparison
1) prepandemic - retro -> most interesting (in analysis needs to be adjusted for time difference between prepandemic and coping sign up)
2) prepandemic - base
3) retro - base


+++ ADD a case_when CHUNKs - who has what data - to get accurate Ns. 
1) binary coded has only prepandemic == 0
2) prepandemic and baseline == 1
3) prepandemic, baseline, and retrospective == 2

+++ chunks for missingness
1) per individual
2) per questionnaire item/total questionnaire


# COPING GLAD: PCL-6

```{r coping glad pcl cols}
coping.glad.pcl.raw <- readRDS(file = "../data_raw/coping_glad/pcl_coping_glad.rds")
...
Retro for the PCL does not mean retrospective
...
coping.glad.pcl.raw.id <- coping.glad.pcl.raw %>%
  ...
  select(
    ...
    pcl.difficulty_concentratin_retro = pcl6.difficulty_concentrating.1
  ) %>%
  mutate_if(is.numeric, ~na_if(., -88)) %>% # Recode missing values to NAs in numeric variables
  mutate_if(is.numeric, ~na_if(., -99)) %>%
  mutate_if(is.numeric, ~na_if(., -77)) %>%
...
coping.glad.phq.raw <- readRDS(file = "../data_raw/coping_glad/phq_coping_glad.rds")
...
Check duplicates again to be save
...
#Exclusion
Exclude those without sex, age and incomplete PHQ/GAD items
Create a new new object for each exclusion step, e.g., data.no_sex and data.no_sex.no_age
...
dim(dat.no.sex)
dat.no.sex.no.age <- dat.no.sex %>%
  drop_na(age_category)
dim(dat.no.sex.no.age)

dim(dat.no.sex)[1]-dim(dat.no.sex.no.age)[1]
```
`r dim(dat.no.sex)[1]-dim(dat.no.sex.no.age)[1]` were excluded because they did not report their age

Exclusion of inflammatory bowel disease (IBD) BioResource (BR) cohort
+++CH: or alternatively sample according to IBD prevalence in UK population
+++CH: What is the IBD prevalence in the UK population? -> Literature search
+++CH: This chunk needs to be included when we have the data available from the NBR
...
dat.no.sex.no.age.no.ibd <- dat.no.sex.no.age
```
...
Create tidy data on which the exclusionc criteria have been applied
```{r Tidy data}
dat.glad <- dat.no.sex.no.age.no.ibd.incomplete.questionnaire

skimr::skim(dat.glad)
```
The complete rate for age and sex should be 1 now


# Definition of prepandemic and baseline 
Exlcude people who have joined GLAD/EDGI after the pandemic began (i.e., cut off that we choose)
Potentially GLAD/EDGI particiapnts between January to April. EDGI launched on February 24th. 

These participants with sign ups later than these dates could  potentially be counted as COPING baseline.
...
+++CH: Create a column based on the the startDate if the participant can be counted as "prepandemic"
+++CH: Create three columns for all the definitions of "pre-pandemic" for sensitivy analysis

Potential dates for the definition of prepandemic
...
What proportion is over a year?
...
dat.respirotry.ramp <- dat.respirtaory.ramp %>%
		  mutate(
		    gender_binary_ramp =
		      case_when(
		        Gender == "Male" ~ "Male",
...
++KT: Need help identifying cut offs for COVID probability
```{r eval=FALSE, include=FALSE}
## COVID prediction model for COPING

## Formula to calculate probability 
dat.respirotry.coping <- dat.respirtaory.coping %>%
		  mutate(
		    covid_probability = 
		      (-1.32) - 
		      (0.01*Age) + 
...
## label the variable
covid_odds_ratio <- "COVID-19 probability" #IGNORE NEED TO EDIT TO LABEL THE ABOVE VARIABLE
...
dat.respirotry.coping <- dat.respirtaory.coping %>%
		  mutate(
		    covid_odds_ratio_50_numeric = 
		      case_when(
		        covid_odds_ratio >= 0 & covid_odds_ratio <= 0.4 ~ "0", #I don't know how to categorise these - how does the previous function know when to cut?
		        covid_odds_ratio >= 0.4 & covid_odds_ratio <= 0.6 ~ "1",
		        covid_odds_ratio >= 0.7 & covid_odds_ratio <= 1 ~ "2"
		      )
...
descr(x = dat.respirotry.coping[,covid_variables],
      round.digits = 2,
      transpose = TRUE)

## create a new label for the variable (50%)
covid_odds_ratio_50 <- "COVID-19 probability >= 50%" #Need to rename variable properly

## looking at the created variable
table(dat.respirotry.coping$covid_odds_ratio_50) #same as before - not sure why this is needed? 
```


Gerome: We can get actual test data from PHE -> Cambridge
We need to move our data to Cambridge cluster
That's only for non-RAMP; EDGI, GLAD; NBR
KY: Longer term or for this paper?
Sanity check with Cambridge about the validity of the mysterious formula (Spector magic)
