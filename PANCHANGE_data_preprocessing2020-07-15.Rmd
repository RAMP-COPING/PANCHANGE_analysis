...
## GLAD and EDGI
### Create data frame
```{r GLAD and EDGI combined}
if(GLAD_and_EDGI == TRUE){
glad.and.edgi.dem.raw.id <- rbind(glad.dem.raw.id, edgi.dem.raw.id)
}
```

```{r glad and edgi copy dem}
if(GLAD_and_EDGI == TRUE){
  dem.raw.id <- glad.and.edgi.dem.raw.id
}
```

### Get summary of combined GLAD and EDI
```{r glad and edgi demographics summary}
if(GLAD_and_EDGI == TRUE) {
summarytools::dfSummary(
  dem.raw.id,
  graph.col = F,
...
```{r ethnicity recoding for GLAD and EDGI combined}
if(GLAD_and_EDGI == TRUE) {
...
glad.employment.raw.id <- glad.employment.raw %>%
  ...
  select(
         ...
         employment.government_work_key_workers, # key worker status
         employment.unemployed, #key worker routing variable
         employment.zerohours_contract, #key worker routing variable
         employment.stayathome_parent_or_carer, #key worker routing variable
         employment.retired, #key worker routing variable
         employment.student_gcse_or_a_level, #key worker routing variable
         employment.student_university, #key worker routing variable
         employment.became_unemployed, #key worker routing variable
         employment.furloughed_or_paid_leave_government_funded, #key worker routing variable
         employment.furloughed_or_paid_leave_company_funded, #key worker routing variable
         employment.paid_leave_furloughed, #key worker routing variable
         employment.fulltime_employed,
         employment.parttime_employed,
         employment.selfemployed,
         employment.contract_or_freelance_work,
         employment.small_business_owner,
         employment.receiving_state_income,
         employment.my_employment_status_has_not_changed,
         employment.reduction_in_hours,
         employment.reduction_in_salary,
         employment.benefits_increased,
         employment.benefits_decreased,
         employment.became_employed,
         employment.became_unemployed,
         employment.increased_hours,
         employment.increased_salary,
                  ) %>%
...
## Employment prior to pandemic (baseline, GLAD, EDGI, NBR)
Proceed along qualifications fields. If field is not blank, take that as highest qual. If field is blank, check next field. If all fields are blank, NA
```{r Employment prior to pandemic baseline}
if(GLAD == TRUE | EDGI == TRUE | NBR == TRUE) {
#create numeric version of the highest education variable
employment.raw.id <- employment.raw.id %>%
  mutate(
    employment_prior_covid_numeric =
      case_when(
        employment.fulltime_employed_numeric == "1" ~ 1,
        employment.parttime_employed_numeric == "1" ~ 1,
        employment.unemployed_numeric == "1" ~ 2,
        employment.zerohours_contract_numeric == "1" ~ 1,
        employment.selfemployed_numeric == "1" ~ 1,
        employment.contract_or_freelance_work_numeric == "1" ~ 1,
        employment.small_business_owner_numeric == "1" ~ 1,
        employment.retired_numeric  ==  "1" ~ 4,
        employment.student_gcse_or_a_level_numeric == "1" ~ 3,
        employment.student_university_numeric == "1" ~ 3
      )
    )
...
```{r Employment prior to pandemic check}
if(GLAD == TRUE | EDGI == TRUE | NBR == TRUE) {
employment.raw.id %>%
  select(employment_prior_covid,
         employment_prior_covid_numeric,
        employment.fulltime_employed_numeric,
        employment.parttime_employed_numeric,
        employment.unemployed_numeric,
        employment.zerohours_contract_numeric,
        employment.selfemployed_numeric,
        employment.contract_or_freelance_work_numeric,
        employment.small_business_owner_numeric,
        employment.retired_numeric,
        employment.student_gcse_or_a_level_numeric,
        employment.student_university_numeric
  )
}
```
...
## Employment change (baseline, GLAD, EDGI, NBR)
Proceed along qualifications fields. If field is not blank, take that as highest qual. If field is blank, check next field. If all fields are blank, NA
```{r Employment change baseline}
if(GLAD == TRUE | EDGI == TRUE | NBR == TRUE) {
#create numeric version of the highest education variable
employment.raw.id <- employment.raw.id %>%
  mutate(
    employment_change_numeric =
      case_when(
        employment.reduction_in_hours_numeric == "1" ~ 1,
        employment.reduction_in_salary_numeric == "1" ~ 1,
        employment.became_unemployed_numeric == "1" ~ 1,
        employment.benefits_decreased_numeric == "1" ~ 1,
        employment.increased_hours_numeric == "1" ~ 2,
        employment.increased_salary_numeric == "1" ~ 3,
        employment.became_employed_numeric == "1" ~ 3,
        employment.benefits_increased_numeric  ==  "1" ~ 2,
        employment.furloughed_or_paid_leave_government_funded_numeric == "1" ~ 3,
        employment.furloughed_or_paid_leave_company_funded_numeric == "1" ~ 3,
        employment.paid_leave_furloughed_numeric == "1" ~ 3,
        employment.my_employment_status_has_not_changed_numeric == "1" ~ 4
      )
    )
...
employment.raw.id %>%
  select(employment_change,
         employment_change_numeric,
        employment.reduction_in_hours_numeric,
        employment.reduction_in_salary_numeric,
        employment.became_unemployed_numeric,
        employment.benefits_decreased_numeric,
        employment.increased_hours_numeric,
        employment.increased_salary_numeric,
        employment.became_employed_numeric,
        employment.benefits_increased_numeric,
        employment.furloughed_or_paid_leave_government_funded_numeric,
        employment.furloughed_or_paid_leave_company_funded_numeric,
        employment.paid_leave_furloughed_numeric,
        employment.my_employment_status_has_not_changed_numeric
  )
}
```
  ...
  mutate(
    key_worker_numeric =
      case_when(
        ...
        employment.unemployed_numeric == "1" ~ 0,
        employment.stayathome_parent_or_carer_numeric == "1" ~ 0,
        employment.retired_numeric == "1" ~ 0,
        employment.became_unemployed_numeric == "0" ~ 0,
        employment.student_gcse_or_a_level_numeric == "1" ~ 0,
        employment.student_university_numeric == "1" ~ 0,
        employment.furloughed_or_paid_leave_government_funded_numeric == "1" ~ 0,
        employment.furloughed_or_paid_leave_company_funded_numeric == "1" ~ 0,
        employment.paid_leave_furloughed_numeric == "1" ~ 0
      )
    )
...
edgi.mhd.raw.id <- edgi.mhd.raw %>%
  ...
  select(
    ...
    mhd.none_of_the_above_eating_disorder = mhd.none_of_the_above,
    mhd.dont_know_eating_disorder = mhd.dont_know,
    mhd.prefer_not_to_answer_eating_disorder = mhd.prefer_not_to_answer
  ) %>%
  add_numeric(., exclude = exclude_cols) %>%
  mutate_if(is.numeric, ~na_if(., -88)) %>% # Recode missing values to NAs in numeric variables
  mutate_if(is.numeric, ~na_if(., -99)) %>%
...
if(EDGI == TRUE){
  ...
  edgi.mhd2.raw.id <- edgi.mhd2.raw %>%
    ...
    mhd.body_dysmorphic_disorder_bdd = demographics.body_dysmorphic_disorder_bdd,
    ...
    mhd.prefer_not_to_answer = demographics.prefer_not_to_answer.3,
    mhd.none_of_the_above.1 = demographics.none_of_the_above.8,
    mhd.dont_know.1 = demographics.dont_know.7,
    mhd.prefer_not_to_answer.1 = demographics.prefer_not_to_answer.11,
    mhd.other_please_tell_us_more = demographics.other.1
    ) %>%
    add_column(
...
nbr.mhd.raw.id <- nbr.mhd.raw %>%
  ...
  select(
    ...
    mhd.none_of_the_above_eating_disorder_unc = mhd.none_of_the_above.2,
...
```{r EDGI mhd restricting_vs_binge_eating_eating_disorders}
if(EDGI == TRUE) {
mhd.raw.id <- mhd.raw.id %>%
  mutate(
    restricting_vs_binge_eating_eating_disorders_numeric =
...
```{r ocir prepan sum score if missing items set to NA - all items}
if(GLAD == TRUE | EDGI == TRUE) {
prepan.ocir.raw.id <- prepan.ocir.raw.id %>%
  mutate(
...
# Create vector with items that are included in total score
ocir.items_retro = c(
  "ocir.i_repeatedly_check_doors_windows_drawers_etc_retro_numeric",
  "ocir.arranged_things_change_upset_retro_numeric",
  "ocir.repeat_feel_numbers_retro_numeric",
  "ocir.clean_simply_wash_feel_retro_numeric",
  "ocir.mind_upset_unpleasant_thoughts_retro_numeric",
  "ocir.afraid_avoid_throwing_things_retro_numeric",
  "ocir.light_switches_water_taps_retro_numeric",
  "ocir.arranged_things_retro_numeric",
  "ocir.good_feel_bad_numbers_retro_numeric",
  "ocir.hands_longer_wash_retro_numeric",
  "ocir.saved_things_retro_numeric",
  "ocir.frequently_difficulty_rid_nasty_retro_numeric",
  "ocir.i_check_things_more_often_than_necessary_retro_numeric",
  "ocir.arranged_properly_objects_upset_retro_numeric",
  "ocir.feel_compelled_count_things_retro_numeric",
  "ocir.strangers_touch_touched_difficult_retro_numeric",
  "ocir.control_difficult_find_thoughts_retro_numeric",
  "ocir.i_collect_things_i_dont_need_retro_numeric"
)
...
# Create vector with items that are included in total score
ocir.items_base = c(
  "ocir.i_repeatedly_check_doors_windows_drawers_etc_base_numeric",
  "ocir.arranged_things_change_upset_base_numeric",
  "ocir.repeat_feel_numbers_base_numeric",
  "ocir.clean_simply_wash_feel_base_numeric",
  "ocir.mind_upset_unpleasant_thoughts_base_numeric",
  "ocir.afraid_avoid_throwing_things_base_numeric",
  "ocir.light_switches_water_taps_base_numeric",
  "ocir.arranged_things_base_numeric",
  "ocir.good_feel_bad_numbers_base_numeric",
  "ocir.hands_longer_wash_base_numeric",
  "ocir.saved_things_base_numeric",
  "ocir.frequently_difficulty_rid_nasty_base_numeric",
  "ocir.i_check_things_more_often_than_necessary_base_numeric",
  "ocir.arranged_properly_objects_upset_base_numeric",
  "ocir.feel_compelled_count_things_base_numeric",
  "ocir.strangers_touch_touched_difficult_base_numeric",
  "ocir.control_difficult_find_thoughts_base_numeric",
  "ocir.i_collect_things_i_dont_need_base_numeric"
)
...
dim(nbr.als.raw.id)
# Inspect colnames
colnames(nbr.als.raw.id)
#Differences
dim(nbr.als.raw)[1]-dim(nbr.als.raw.id)[1]
}
```

```{r nbr copy als}
if(NBR == TRUE){
als.raw.id <- nbr.als.raw.id
}
```
...
```{r ramp als cols}
if(RAMP == TRUE){
ramp.als.raw <- readRDS(file = paste0(data.raw_path, "/ramp/alsfrs_ramp.rds"))
dim(ramp.als.raw)
colnames(ramp.als.raw)
}
```
...
ramp.als.raw.id <- ramp.als.raw %>%
  ...
  select(
         Sample, # Sample
         ID, # ID
         alsfrs.limb_weakness_noticed_past = alsfrs.limb_weakness_past_month,
         alsfrs.normal,
         alsfrs.slow_or_sloppy_all_words_are_legible,
         alsfrs.not_all_words_are_legible,
         alsfrs.able_to_grip_pen,
...
dim(ramp.als.raw.id)
# Inspect colnames
colnames(ramp.als.raw.id)
#Differences
dim(ramp.als.raw)[1]-dim(ramp.als.raw.id)[1]
}
```

```{r ramp copy als}
if(RAMP == TRUE){
als.raw.id <- ramp.als.raw.id
}
```
...
```{r glad ess cols}
if(GLAD == TRUE){
glad.ess.raw <- readRDS(file = paste0(data.raw_path, "/glad/ess_coping_glad.rds"))
dim(glad.ess.raw)
colnames(glad.ess.raw)
}
```
...
glad.ess.raw.id <- glad.ess.raw %>%
  ...
  select(
         Sample, # Sample
         ID, # ID
         ess.felt_excessively_sleepy_past,
         ess.sitting_and_reading,
         ess.watching_tv,
         ess.sitting_inactive_in_a_public_place,
         ess.break_passenger_car_hour,
         ess.afternoon_rest_circumstances_lying,
         ess.sitting_and_talking_to_someone,
         ess.sitting_quietly_after_a_lunch_without_alcohol,
         ess.car_minutes_stopped_traffic,
         ess.insomnia,
         ess.narcolepsy,
         ess.rem_sleep_behaviour_disorder,
         ess.restless_legs_syndrome,
         ess.sleep_apnea,
         ess.sleep_terrors,
         ess.sleepwalking,
         ess.other,
...
dim(glad.ess.raw.id)
# Inspect colnames
colnames(glad.ess.raw.id)
#Differences
dim(glad.ess.raw)[1]-dim(glad.ess.raw.id)[1]
}
```

```{r glad copy ess}
if(GLAD == TRUE){
ess.raw.id <- glad.ess.raw.id
}
```
...
```{r edgi ess cols}
if(EDGI == TRUE){
edgi.ess.raw <- readRDS(file = paste0(data.raw_path, "/edgi/ess_coping_edgi.rds"))
dim(edgi.ess.raw)
colnames(edgi.ess.raw)
}
```
...
edgi.ess.raw.id <- edgi.ess.raw %>%
  ...
  select(
         ...
         ess.watching_tv,
         ess.sitting_inactive_in_a_public_place,
         ess.break_passenger_car_hour = ess.break_passenger_hour_car,
         ess.afternoon_rest_circumstances_lying,
         ess.sitting_and_talking_to_someone,
         ess.sitting_quietly_after_a_lunch_without_alcohol,
         ess.car_minutes_stopped_traffic,
...
dim(edgi.ess.raw.id)
# Inspect colnames
colnames(edgi.ess.raw.id)
#Differences
dim(edgi.ess.raw)[1]-dim(edgi.ess.raw.id)[1]
}
```

```{r edgi copy ess}
if(EDGI == TRUE){
ess.raw.id <- edgi.ess.raw.id
}
```
...
```{r nbr ess cols}
if(NBR == TRUE){
nbr.ess.raw <- readRDS(file = paste0(data.raw_path, "/nbr/ess_coping_nbr.rds"))
dim(nbr.ess.raw)
colnames(nbr.ess.raw)
}
```
...
dim(nbr.ess.raw.id)
# Inspect colnames
colnames(nbr.ess.raw.id)
#Differences
dim(nbr.ess.raw)[1]-dim(nbr.ess.raw.id)[1]
}
```

```{r nbr copy ess}
if(NBR == TRUE){
ess.raw.id <- nbr.ess.raw.id
}
```
...
```{r ramp ess cols}
if(RAMP == TRUE){
ramp.ess.raw <- readRDS(file = paste0(data.raw_path, "/ramp/ess_ramp.rds"))
dim(ramp.ess.raw)
colnames(ramp.ess.raw)
}
```
...
ramp.ess.raw.id <- ramp.ess.raw %>%
  ...
  select(
         Sample, # Sample
         ID, # ID
         ess.felt_excessively_sleepy_past = ess.felt_excessively_sleepy_day,
         ess.sitting_and_reading,
         ess.watching_tv,
         ess.sitting_inactive_in_a_public_place,
         ess.break_passenger_car_hour = ess.break_hour_passenger_car,
         ess.afternoon_rest_circumstances_lying,
         ess.sitting_and_talking_to_someone,
         ess.sitting_quietly_after_a_lunch_without_alcohol,
         ess.car_minutes_stopped_traffic#,
#         ess.insomnia, # questions are not asked in RAMP
#         ess.narcolepsy,
#         ess.rem_sleep_behaviour_disorder,
#         ess.restless_legs_syndrome,
#         ess.sleep_apnea,
...
#         ess.none
  ...
  add_numeric(., exclude = exclude_cols) %>%
    add_column(
    ess.insomnia = NA_character_,
    ess.narcolepsy = NA_character_,
    ess.rem_sleep_behaviour_disorder = NA_character_,
    ess.restless_legs_syndrome = NA_character_,
    ess.sleep_apnea = NA_character_,
    ess.sleep_terrors = NA_character_,
    ess.sleepwalking = NA_character_,
    ess.other = NA_character_,
    ess.none = NA_character_,
    ess.insomnia_numeric = NA_real_,
    ess.narcolepsy_numeric = NA_real_,
    ess.rem_sleep_behaviour_disorder_numeric = NA_real_,
    ess.restless_legs_syndrome_numeric = NA_real_,
    ess.sleep_apnea_numeric = NA_real_,
    ess.sleep_terrors_numeric = NA_real_,
    ess.sleepwalking_numeric = NA_real_,
    ess.other_numeric = NA_real_,
    ess.none_numeric = NA_real_
    ) %>%
  mutate_if(is.numeric, ~na_if(., -88)) %>% # Recode missing values to NAs in numeric variables
  mutate_if(is.numeric, ~na_if(., -99)) %>%
  mutate_if(is.numeric, ~na_if(., -77)) %>%
...
dim(ramp.ess.raw.id)
# Inspect colnames
colnames(ramp.ess.raw.id)
#Differences
dim(ramp.ess.raw)[1]-dim(ramp.ess.raw.id)[1]
}
```

```{r ramp copy ess}
if(RAMP == TRUE){
ess.raw.id <- ramp.ess.raw.id
}
```
...
```{r glad midas cols}
if(GLAD == TRUE){
glad.midas.raw <- readRDS(file = paste0(data.raw_path, "/glad/midas_coping_glad.rds"))
dim(glad.midas.raw)
colnames(glad.midas.raw)
}
```

Select variables
```{r glad midas cleaning}
if(GLAD == TRUE){

glad.midas.raw.id <- glad.midas.raw %>%
  drop_na(externalDataReference) %>% # Drop NAs
  distinct(externalDataReference, .keep_all = TRUE) %>% # Remove duplicates based on ID
  separate(externalDataReference, into = c("Sample", "ID"), sep = 4) %>% # Split ID in Sample and Number
  mutate(ID = as.numeric(ID)) %>%
  select(
         Sample, # Sample
         ID, # ID
         midas.headaches_suffered_past_month,
         midas.days_past_month_miss_work_school = midas.past_month_days,
         midas.days_past_month_work_school_half_productivity = midas.past_month_days.1,
         midas.days_past_month_miss_housework = midas.past_month_days.2,
         midas.days_past_month_housework_half_productivity = midas.past_month_days.3,
         midas.days_past_month_miss_social_activities = midas.past_month_days.4
         ) %>%
  add_numeric(., exclude = exclude_cols) %>%
  mutate_if(is.numeric, ~na_if(., -88)) %>% # Recode missing values to NAs in numeric variables
  mutate_if(is.numeric, ~na_if(., -99)) %>%
...
# Inspect dimensions
dim(glad.midas.raw.id)
# Inspect colnames
colnames(glad.midas.raw.id)
#Differences
dim(glad.midas.raw)[1]-dim(glad.midas.raw.id)[1]
}
```

```{r glad copy midas}
if(GLAD == TRUE){
midas.raw.id <- glad.midas.raw.id
}
```
...
```{r edgi midas cols}
if(EDGI == TRUE){
edgi.midas.raw <- readRDS(file = paste0(data.raw_path, "/edgi/midas_coping_edgi.rds"))
dim(edgi.midas.raw)
colnames(edgi.midas.raw)
}
```

Select variables
```{r edgi midas cleaning}
if(EDGI == TRUE){

edgi.midas.raw.id <- edgi.midas.raw %>%
  drop_na(externalDataReference) %>% # Drop NAs
  distinct(externalDataReference, .keep_all = TRUE) %>% # Remove duplicates based on ID
  separate(externalDataReference, into = c("Sample", "ID"), sep = 4) %>% # Split ID in Sample and Number
  mutate(ID = as.numeric(ID)) %>%
...
# Inspect dimensions
dim(edgi.midas.raw.id)
# Inspect colnames
colnames(edgi.midas.raw.id)
#Differences
dim(edgi.midas.raw)[1]-dim(edgi.midas.raw.id)[1]
}
```

```{r edgi copy midas}
if(EDGI == TRUE){
midas.raw.id <- edgi.midas.raw.id
}
```
...
```{r nbr midas cols}
if(NBR == TRUE){
nbr.midas.raw <- readRDS(file = paste0(data.raw_path, "/nbr/midas_coping_nbr.rds"))
dim(nbr.midas.raw)
colnames(nbr.midas.raw)
}
```

```{r nbr midas summary labels}
if(NBR == TRUE) {
summarytools::dfSummary(
  nbr.midas.raw,
  graph.col = F,
...
Select variables
```{r nbr midas cleaning}
if(NBR == TRUE){

nbr.midas.raw.id <- nbr.midas.raw %>%
  drop_na(subjectid) %>% # Drop NAs
  distinct(subjectid, .keep_all = TRUE) %>% # Remove duplicates based on ID
  separate(subjectid, into = c("Sample", "ID.intm"), sep = 6) %>% # Split ID in Sample and Number
  separate(ID.intm, into = "ID", sep = 7) %>%
...
# Inspect dimensions
dim(nbr.midas.raw.id)
# Inspect colnames
colnames(nbr.midas.raw.id)
#Differences
dim(nbr.midas.raw)[1]-dim(nbr.midas.raw.id)[1]
}
```

```{r nbr copy midas}
if(NBR == TRUE){
midas.raw.id <- nbr.midas.raw.id
}
```
...
```{r ramp midas cols}
if(RAMP == TRUE){
ramp.midas.raw <- readRDS(file = paste0(data.raw_path, "/ramp/midas_ramp.rds"))
dim(ramp.midas.raw)
colnames(ramp.midas.raw)
}
```

```{r ramp midas summary labels}
if(RAMP == TRUE) {
summarytools::dfSummary(
  ramp.midas.raw,
  graph.col = F,
...
Select variables
```{r ramp midas cleaning}
if(RAMP == TRUE){

ramp.midas.raw.id <- ramp.midas.raw %>%
  drop_na(`Login ID`) %>% # Drop NAs
  distinct(`Login ID`, .keep_all = TRUE) %>% # Remove duplicates based on ID
  separate(`Login ID`, into = c("Sample", "ID"), sep = 4) %>% # Split ID in Sample and Number
  mutate(ID = as.numeric(ID)) %>%
  select(
         ...
         ID, # ID
         midas.headaches_suffered_past_month,
         midas.days_past_month_miss_work_school = midas.scale_line_click_firmly,
         midas.days_past_month_work_school_half_productivity = midas.scale_line_click_firmly.1,
         midas.days_past_month_miss_housework = midas.scale_line_click_firmly.2,
         midas.days_past_month_housework_half_productivity = midas.scale_line_click_firmly.3,
         midas.days_past_month_miss_social_activities = midas.scale_line_click_firmly.4
         ) %>%
  add_numeric(., exclude = exclude_cols) %>%
  mutate_if(is.numeric, ~na_if(., -88)) %>% # Recode missing values to NAs in numeric variables
  mutate_if(is.numeric, ~na_if(., -99)) %>%
...
# Inspect dimensions
dim(ramp.midas.raw.id)
# Inspect colnames
colnames(ramp.midas.raw.id)
#Differences
dim(ramp.midas.raw)[1]-dim(ramp.midas.raw.id)[1]
}
```

```{r ramp copy midas}
if(RAMP == TRUE){
midas.raw.id <- ramp.midas.raw.id
}
```
...
```{r glad nmss cols}
if(GLAD == TRUE){
glad.nmss.raw <- readRDS(file = paste0(data.raw_path, "/glad/nmss_coping_glad.rds"))
dim(glad.nmss.raw)
colnames(glad.nmss.raw)
}
```

Select variables
```{r glad nmss cleaning}
if(GLAD == TRUE){

glad.nmss.raw.id <- glad.nmss.raw %>%
  drop_na(externalDataReference) %>% # Drop NAs
  distinct(externalDataReference, .keep_all = TRUE) %>% # Remove duplicates based on ID
  separate(externalDataReference, into = c("Sample", "ID"), sep = 4) %>% # Split ID in Sample and Number
  mutate(ID = as.numeric(ID)) %>%
  select(
         Sample, # Sample
         ID, # ID
         nmss.ability_taste_food_smell,
         nmss.severe_taste_smell_loss
         ) %>%
  add_numeric(., exclude = exclude_cols) %>%
  mutate_if(is.numeric, ~na_if(., -88)) %>% # Recode missing values to NAs in numeric variables
  mutate_if(is.numeric, ~na_if(., -99)) %>%
...
# Inspect dimensions
dim(glad.nmss.raw.id)
# Inspect colnames
colnames(glad.nmss.raw.id)
#Differences
dim(glad.nmss.raw)[1]-dim(glad.nmss.raw.id)[1]
}
```

```{r glad copy nmss}
if(GLAD == TRUE){
nmss.raw.id <- glad.nmss.raw.id
}
```
...
```{r edgi nmss cols}
if(EDGI == TRUE){
edgi.nmss.raw <- readRDS(file = paste0(data.raw_path, "/edgi/nmss_coping_edgi.rds"))
dim(edgi.nmss.raw)
colnames(edgi.nmss.raw)
}
```

Select variables
```{r edgi nmss cleaning}
if(EDGI == TRUE){

edgi.nmss.raw.id <- edgi.nmss.raw %>%
  drop_na(externalDataReference) %>% # Drop NAs
  distinct(externalDataReference, .keep_all = TRUE) %>% # Remove duplicates based on ID
  separate(externalDataReference, into = c("Sample", "ID"), sep = 4) %>% # Split ID in Sample and Number
  mutate(ID = as.numeric(ID)) %>%
  select(
         Sample, # Sample
         ID, # ID
         nmss.ability_taste_food_smell = nmss.taste_food_ability_smell, # different to glad
         nmss.severe_taste_smell_loss
         ) %>%
  add_numeric(., exclude = exclude_cols) %>%
  mutate_if(is.numeric, ~na_if(., -88)) %>% # Recode missing values to NAs in numeric variables
...
# Inspect dimensions
dim(edgi.nmss.raw.id)
# Inspect colnames
colnames(edgi.nmss.raw.id)
#Differences
dim(edgi.nmss.raw)[1]-dim(edgi.nmss.raw.id)[1]
}
```

```{r edgi copy nmss}
if(EDGI == TRUE){
nmss.raw.id <- edgi.nmss.raw.id
}
```
...
```{r nbr nmss cols}
if(NBR == TRUE){
nbr.nmss.raw <- readRDS(file = paste0(data.raw_path, "/nbr/nmss_coping_nbr.rds"))
dim(nbr.nmss.raw)
colnames(nbr.nmss.raw)
}
```
...
dim(nbr.nmss.raw.id)
# Inspect colnames
colnames(nbr.nmss.raw.id)
#Differences
dim(nbr.nmss.raw)[1]-dim(nbr.nmss.raw.id)[1]
}
```

```{r nbr copy nmss}
if(NBR == TRUE){
nmss.raw.id <- nbr.nmss.raw.id
}
```
...
```{r ramp nmss cols}
if(RAMP == TRUE){
ramp.nmss.raw <- readRDS(file = paste0(data.raw_path, "/ramp/nmss_ramp.rds"))
dim(ramp.nmss.raw)
colnames(ramp.nmss.raw)
}
```

Select variables
```{r ramp nmss cleaning}
if(RAMP == TRUE){

ramp.nmss.raw.id <- ramp.nmss.raw %>%
  drop_na(`Login ID`) %>% # Drop NAs
  distinct(`Login ID`, .keep_all = TRUE) %>% # Remove duplicates based on ID
  separate(`Login ID`, into = c("Sample", "ID"), sep = 4) %>% # Split ID in Sample and Number
  mutate(ID = as.numeric(ID)) %>%
  select(
         ...
         ID, # ID
         nmss.ability_taste_food_smell,
         nmss.severe_taste_smell_loss = nmss.severe_taste_loss_smell
         ) %>%
  add_numeric(., exclude = exclude_cols) %>%
  mutate_if(is.numeric, ~na_if(., -88)) %>% # Recode missing values to NAs in numeric variables
  mutate_if(is.numeric, ~na_if(., -99)) %>%
...
# Inspect dimensions
dim(ramp.nmss.raw.id)
# Inspect colnames
colnames(ramp.nmss.raw.id)
#Differences
dim(ramp.nmss.raw)[1]-dim(ramp.nmss.raw.id)[1]
}
```

```{r ramp copy nmss}
if(RAMP == TRUE){
nmss.raw.id <- ramp.nmss.raw.id
}
```
...
```{r glad nmsw cols}
if(GLAD == TRUE){
glad.nmsw.raw <- readRDS(file = paste0(data.raw_path, "/glad/nmsw_coping_glad.rds"))
dim(glad.nmsw.raw)
colnames(glad.nmsw.raw)
}
```

Select variables
```{r glad nmsw cleaning}
if(GLAD == TRUE){

glad.nmsw.raw.id <- glad.nmsw.raw %>%
  drop_na(externalDataReference) %>% # Drop NAs
  distinct(externalDataReference, .keep_all = TRUE) %>% # Remove duplicates based on ID
  separate(externalDataReference, into = c("Sample", "ID"), sep = 4) %>% # Split ID in Sample and Number
  mutate(ID = as.numeric(ID)) %>%
  select(
         Sample, # Sample
         ID, # ID
         nmsw.drink_problems_choking_difficulty,
         nmsw.regularly_night_pass_urine,
         nmsw.sex_difficult_finding,
         nmsw.weak_standing_feeling_light,
         nmsw.acting_moving_sleep_talking,
         nmsw.double_vision
         ) %>%
  add_numeric(., exclude = exclude_cols) %>%
  mutate_if(is.numeric, ~na_if(., -88)) %>% # Recode missing values to NAs in numeric variables
...
# Inspect dimensions
dim(glad.nmsw.raw.id)
# Inspect colnames
colnames(glad.nmsw.raw.id)
#Differences
dim(glad.nmsw.raw)[1]-dim(glad.nmsw.raw.id)[1]
}
```

```{r glad copy nmsw}
if(GLAD == TRUE){
nmsw.raw.id <- glad.nmsw.raw.id
}
```
...
```{r edgi nmsw cols}
if(EDGI == TRUE){
edgi.nmsw.raw <- readRDS(file = paste0(data.raw_path, "/edgi/nmsw_coping_edgi.rds"))
dim(edgi.nmsw.raw)
colnames(edgi.nmsw.raw)
}
```

Select variables
```{r edgi nmsw cleaning}
if(EDGI == TRUE){

edgi.nmsw.raw.id <- edgi.nmsw.raw %>%
  drop_na(externalDataReference) %>% # Drop NAs
  distinct(externalDataReference, .keep_all = TRUE) %>% # Remove duplicates based on ID
  separate(externalDataReference, into = c("Sample", "ID"), sep = 4) %>% # Split ID in Sample and Number
  mutate(ID = as.numeric(ID)) %>%
...
# Inspect dimensions
dim(edgi.nmsw.raw.id)
# Inspect colnames
colnames(edgi.nmsw.raw.id)
#Differences
dim(edgi.nmsw.raw)[1]-dim(edgi.nmsw.raw.id)[1]
}
```

```{r edgi copy nmsw}
if(EDGI == TRUE){
nmsw.raw.id <- edgi.nmsw.raw.id
}
```
...
```{r nbr nmsw cols}
if(NBR == TRUE){
nbr.nmsw.raw <- readRDS(file = paste0(data.raw_path, "/nbr/nmsw_coping_nbr.rds"))
dim(nbr.nmsw.raw)
colnames(nbr.nmsw.raw)
}
```
...
dim(nbr.nmsw.raw.id)
# Inspect colnames
colnames(nbr.nmsw.raw.id)
#Differences
dim(nbr.nmsw.raw)[1]-dim(nbr.nmsw.raw.id)[1]
}
```

```{r nbr copy nmsw}
if(NBR == TRUE){
nmsw.raw.id <- nbr.nmsw.raw.id
}
```
...
```{r ramp nmsw cols}
if(RAMP == TRUE){
ramp.nmsw.raw <- readRDS(file = paste0(data.raw_path, "/ramp/nmsw_ramp.rds"))
dim(ramp.nmsw.raw)
colnames(ramp.nmsw.raw)
}
```

Select variables
```{r ramp nmsw cleaning}
if(RAMP == TRUE){

ramp.nmsw.raw.id <- ramp.nmsw.raw %>%
  drop_na(`Login ID`) %>% # Drop NAs
  distinct(`Login ID`, .keep_all = TRUE) %>% # Remove duplicates based on ID
  separate(`Login ID`, into = c("Sample", "ID"), sep = 4) %>% # Split ID in Sample and Number
  mutate(ID = as.numeric(ID)) %>%
  select(
         ...
         ID, # ID
         nmsw.drink_problems_choking_difficulty,
         nmsw.regularly_night_pass_urine = nmsw.night_regularly_pass_urine, # different glad
         nmsw.sex_difficult_finding = nmsw.sex_finding_difficult, # different glad
         nmsw.weak_standing_feeling_light = nmsw.weak_standing_dizzy_feeling, # different glad
         nmsw.acting_moving_sleep_talking,
         nmsw.double_vision
         ) %>%
  add_numeric(., exclude = exclude_cols) %>%
...
# Inspect dimensions
dim(ramp.nmsw.raw.id)
# Inspect colnames
colnames(ramp.nmsw.raw.id)
#Differences
dim(ramp.nmsw.raw)[1]-dim(ramp.nmsw.raw.id)[1]
}
```

```{r ramp copy nmsw}
if(RAMP == TRUE){
nmsw.raw.id <- ramp.nmsw.raw.id
}
```
...
### GLAD
Read in data
```{r glad tinnitus cols}
if(GLAD == TRUE){
glad.tinnitus.raw <- readRDS(file = paste0(data.raw_path, "/glad/tinnitus_coping_glad.rds"))
dim(glad.tinnitus.raw)
colnames(glad.tinnitus.raw)
}
```

Select variables
```{r glad tinnitus cleaning}
if(GLAD == TRUE){

glad.tinnitus.raw.id <- glad.tinnitus.raw %>%
  drop_na(externalDataReference) %>% # Drop NAs
  distinct(externalDataReference, .keep_all = TRUE) %>% # Remove duplicates based on ID
  separate(externalDataReference, into = c("Sample", "ID"), sep = 4) %>% # Split ID in Sample and Number
  mutate(ID = as.numeric(ID)) %>%
  select(
         Sample, # Sample
         ID, # ID
         tinnitus.ears_noises_minutes_head,
         tinnitus.ears_noises_minutes_head_frequency = tinnitus.ears_noises_minutes_head.1,
         tinnitus.noises_worry_worst_upset
         ) %>%
  add_numeric(., exclude = exclude_cols) %>%
  mutate_if(is.numeric, ~na_if(., -88)) %>% # Recode missing values to NAs in numeric variables
  mutate_if(is.numeric, ~na_if(., -99)) %>%
...
# Inspect dimensions
dim(glad.tinnitus.raw.id)
# Inspect colnames
colnames(glad.tinnitus.raw.id)
#Differences
dim(glad.tinnitus.raw)[1]-dim(glad.tinnitus.raw.id)[1]
}
```

```{r glad copy tinnitus}
if(GLAD == TRUE){
tinnitus.raw.id <- glad.tinnitus.raw.id
}
```

### EDGI
Read in data
```{r edgi tinnitus cols}
if(EDGI == TRUE){
edgi.tinnitus.raw <- readRDS(file = paste0(data.raw_path, "/edgi/tinnitus_coping_edgi.rds"))
dim(edgi.tinnitus.raw)
colnames(edgi.tinnitus.raw)
}
```

Select variables
```{r edgi tinnitus cleaning}
if(EDGI == TRUE){

edgi.tinnitus.raw.id <- edgi.tinnitus.raw %>%
  drop_na(externalDataReference) %>% # Drop NAs
  distinct(externalDataReference, .keep_all = TRUE) %>% # Remove duplicates based on ID
  separate(externalDataReference, into = c("Sample", "ID"), sep = 4) %>% # Split ID in Sample and Number
  mutate(ID = as.numeric(ID)) %>%
...
# Inspect dimensions
dim(edgi.tinnitus.raw.id)
# Inspect colnames
colnames(edgi.tinnitus.raw.id)
#Differences
dim(edgi.tinnitus.raw)[1]-dim(edgi.tinnitus.raw.id)[1]
}
```

```{r edgi copy tinnitus}
if(EDGI == TRUE){
tinnitus.raw.id <- edgi.tinnitus.raw.id
}
```
...
```{r nbr tinnitus cols}
if(NBR == TRUE){
nbr.tinnitus.raw <- readRDS(file = paste0(data.raw_path, "/nbr/tinnitus_coping_nbr.rds"))
dim(nbr.tinnitus.raw)
colnames(nbr.tinnitus.raw)
}
```

```{r ramp tinnitus summary labels}
if(NBR == TRUE) {
summarytools::dfSummary(
  nbr.tinnitus.raw,
  graph.col = F,
...
Select variables
```{r nbr tinnitus cleaning}
if(NBR == TRUE){

nbr.tinnitus.raw.id <- nbr.tinnitus.raw %>%
  drop_na(subjectid) %>% # Drop NAs
  distinct(subjectid, .keep_all = TRUE) %>% # Remove duplicates based on ID
  separate(subjectid, into = c("Sample", "ID.intm"), sep = 6) %>% # Split ID in Sample and Number
  separate(ID.intm, into = "ID", sep = 7) %>%
...
# Inspect dimensions
dim(nbr.tinnitus.raw.id)
# Inspect colnames
colnames(nbr.tinnitus.raw.id)
#Differences
dim(nbr.tinnitus.raw)[1]-dim(nbr.tinnitus.raw.id)[1]
}
```

```{r nbr copy tinnitus}
if(NBR == TRUE){
tinnitus.raw.id <- nbr.tinnitus.raw.id
}
```

### RAMP
Read in data
```{r ramp tinnitus cols}
if(RAMP == TRUE){
ramp.tinnitus.raw <- readRDS(file = paste0(data.raw_path, "/ramp/tinnitus_ramp.rds"))
dim(ramp.tinnitus.raw)
colnames(ramp.tinnitus.raw)
}
```

Select variables
```{r ramp tinnitus cleaning}
if(RAMP == TRUE){

ramp.tinnitus.raw.id <- ramp.tinnitus.raw %>%
  drop_na(`Login ID`) %>% # Drop NAs
  distinct(`Login ID`, .keep_all = TRUE) %>% # Remove duplicates based on ID
  separate(`Login ID`, into = c("Sample", "ID"), sep = 4) %>% # Split ID in Sample and Number
  mutate(ID = as.numeric(ID)) %>%
  select(
         ...
         ID, # ID
         tinnitus.ears_noises_minutes_head,
         tinnitus.ears_noises_minutes_head_frequency = tinnitus.lasts_ears_noises_minutes,
         tinnitus.noises_worry_worst_upset = tinnitus.noises_worry_worst_annoy
         ) %>%
  add_numeric(., exclude = exclude_cols) %>%
  mutate_if(is.numeric, ~na_if(., -88)) %>% # Recode missing values to NAs in numeric variables
  mutate_if(is.numeric, ~na_if(., -99)) %>%
...
# Inspect dimensions
dim(ramp.tinnitus.raw.id)
# Inspect colnames
colnames(ramp.tinnitus.raw.id)
#Differences
dim(ramp.tinnitus.raw)[1]-dim(ramp.tinnitus.raw.id)[1]
}
```

```{r ramp copy tinnitus}
if(RAMP == TRUE){
tinnitus.raw.id <- ramp.tinnitus.raw.id
}
```
...
dfs.list <- list(
  ...
  coping.pcl.raw.id,
  coping.phq.raw.id,
#  coping.ocir.raw.id, # joint in the chunk before because it was missing the EDGI ID in any column
  als.raw.id,
  ess.raw.id,
  midas.raw.id,
  nmss.raw.id,
...
excluded_no_sex <- dim(dat.raw)[1]-dim(dat.no.sex)[1]

paste("N =", excluded_no_sex, "were excluded because they did not report their sex", sep = " ")
}
```
...
# IMPUTE RETROSPECTIE DATA for no change 
...
```{r time difference between starting the COPING questionnaire and the beginning of the pandemic}
...
```{r time difference between sign up and the beginning of the pandemic}
if(GLAD == TRUE | EDGI == TRUE){
# using date of pandemic as 23rd of March
dat <- dat %>%
  mutate(
    time_diff_signup_lockdown_march_23 =
        as.numeric(
        difftime(           #this function does time1 - time2 as difftime(time1, time2...)
          date_uk_lockdown_march_23,    # should be start date of the pandemic (time1)
          startDate.prepandemic),    # should be start date for sign up questionnaire EDGI or GLAD (time2)
          units = c("weeks")  #can also be "secs", "mins", "hours", "weeks"
          )
        )
...
dat %>%
  descr(
    var = time_diff_signup_lockdown_march_23,
    stats = "common"
    )
...
dat <- dat %>%
  mutate(
    time_diff_signup_awareness_march_1 =
        as.numeric(
        difftime(           #this function does time1 - time2 as difftime(time1, time2...)
          date_higher_awareness_march_1,    # should be start date of the pandemic (time1)
          startDate.prepandemic),    # should be start date for sign up questionnaire EDGI or GLAD (time2)
          units = c("weeks")  #can also be "secs", "mins", "hours", "weeks"
          )
        )
...
dat %>%
  descr(
    var = time_diff_signup_awareness_march_1,
    stats = "common"
    )
...
dat <- dat %>%
  mutate(
    time_diff_signup_first_case_uk_jan_31 =
        as.numeric(
        difftime(           #this function does time1 - time2 as difftime(time1, time2...)
          date_first_case_uk_january_31,    # should be start date of the pandemic (time1)
          startDate.prepandemic),    # should be start date for sign up questionnaire EDGI or GLAD (time2)
          units = c("weeks")  #can also be "secs", "mins", "hours", "weeks"
          )
        )
...
dat %>%
  descr(
    var = time_diff_signup_first_case_uk_jan_31,
    stats = "common"
    )
}
```
...
```{r Testing 3, 6, and 6 months before pandemic}
if(GLAD == TRUE | EDGI == TRUE){
dat %>%
  filter(time_diff_signup_lockdown_march_23 < 12) %>%
  nrow()

dat %>%
  filter(time_diff_signup_lockdown_march_23 < 24) %>%
  nrow()

dat %>%
  filter(time_diff_signup_lockdown_march_23 < 36) %>%
  nrow()

dat %>%
  filter(time_diff_signup_awareness_march_1 < 12) %>%
  nrow()

dat %>%
  filter(time_diff_signup_awareness_march_1 < 24) %>%
  nrow()

dat %>%
  filter(time_diff_signup_awareness_march_1 < 36) %>%
  nrow()

dat %>%
  filter(time_diff_signup_first_case_uk_jan_31 < 12) %>%
  nrow()

dat %>%
  filter(time_diff_signup_first_case_uk_jan_31 < 24) %>%
  nrow()

dat %>%
  filter(time_diff_signup_first_case_uk_jan_31 < 36) %>%
  nrow()
}
```
...
```{r glad/edgi ppts signed up over half a year ago, eval=FALSE, include=FALSE}
if(GLAD == TRUE | EDGI == TRUE){
dat <- dat %>%
  mutate(
...
summary(dat$phq.diff_score_retro_prepan)
...
summary(dat$phq.diff_score_base_prepan)
}
...
summary(dat$gad.diff_score_base_prepan)
}
...
summary(dat$pcl.diff_score_base_prepan)
}
```
...
summary(dat$ocir.diff_score_base_prepan)
}
...
```{r create numeric sex variable with male eq 1}
...
```{r create numeric sex variable with male eq 1 for ramp}
if(RAMP == TRUE){
dat.coping <- dat.coping %>%
  mutate(
    Sex_respiratory =
