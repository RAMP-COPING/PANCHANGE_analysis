...
#      Age_uncleaned = demographics.how_old_are_you_now.txt, # Age
      Birthyear = demographics.year, # Birthyear
      Gender = demographics.which_gender_do_you_identify_with, # Gender
      Sex = demographics.select_questionnaire_items_medical, #Sex
      EduYrs = demographics.school_education_school_years.txt, # Years in education
      demographics.college_or_university_degree, # level of education - univeristy
...
```{r ramp emp cols}
if(RAMP == TRUE){
  ramp.emp.raw <- readRDS(file = paste0(data.raw_path, "/ramp/employment_ramp.rds"))
  dim(ramp.emp.raw)
  colnames(ramp.emp.raw)
}
```
...
```{r ramp emp cleaning}
if(RAMP == TRUE){
  exclude_cols_emp <- c("ID",
                        "Sample")

ramp.emp.raw$externalDataReference <- ramp.emp.raw$`Login ID` #move Login ID to external data reference
  
ramp.emp.raw.id <- ramp.emp.raw %>%
    drop_na(externalDataReference) %>% # Drop NAs
    distinct(externalDataReference, .keep_all = TRUE) %>% # Remove duplicates based on ID
    separate(externalDataReference, into = c("Sample", "ID"), sep = 4) %>% # Split ID in Sample and Number
    mutate(ID = as.numeric(ID),
           startDate.ramp = startDate) %>% # Rename startDate
    select(
      highest_education_ramp = employment.what_is_your_highest_level_of_education,
    ...
    add_numeric(., exclude = exclude_cols_emp) %>%
    mutate_if(is.numeric, ~na_if(., -88)) %>% # Recode missing values to NAs in numeric variables
    mutate_if(is.numeric, ~na_if(., -99)) %>%
    mutate_if(is.numeric, ~na_if(., -77)) %>%
    mutate_if(is.factor, ~na_if(., "Seen but not answered")) %>% # Recode missing values to NAs in factor variables
  ...
  dim(ramp.emp.raw.id)
  # Inspect colnames
  colnames(ramp.emp.raw.id)
  #Differences
  dim(ramp.emp.raw)[1]-dim(ramp.emp.raw.id)[1]
}
```

```{r ramp copy emp}
if(RAMP == TRUE){
  emp.raw.id <- ramp.emp.raw.id
}
```
...
if(GLAD == TRUE | EDGI == TRUE) {
#create numeric version of the highest education variable
glad.employment.raw.id <- glad.employment.raw.id %>%
  mutate(
    highest_education_numeric =
      case_when(
...
```{r glad mhd restricting_vs_binge_eating_eating_disorders}
mhd.raw.id <- mhd.raw.id %>%
  mutate(
...
```{r glad mhd bipolar_and_schizophrenia_numeric}
mhd.raw.id <- mhd.raw.id %>%
  mutate(
