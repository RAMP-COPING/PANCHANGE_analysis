...
*** MD - need to adapt for participants with missing data (should be NA)
...
coping.glad.ocir.raw <- readRDS(file = paste0(data.raw_path, "/2020-08-03/glad/ocir_coping_glad.rds"))
...
# Create vector with scoring key: If the item is reversed coded use -1. 
# There needs to be the correct number of values for each item in the questionnaire. 
ocir.items_key_retro <- c(1,1,1,1,1,1,1,1,1)
...
# Create vector with scoring key: If the item is reversed coded use -1. 
# There needs to be the correct number of values for each item in the questionnaire. 
ocir.items_key_base <- c(1,1,1,1,1,1,1,1,1)

# Create vector with items that are included in total score
ocir.items_base = c(
  "ocir.little_interest_or_pleasure_in_doing_things_base_numeric",
"ocir.feeling_down_depressed_or_hopeless_base_numeric",
"ocir.staying_asleep_sleeping_trouble_base_numeric",
"ocir.feeling_tired_or_having_little_energy_base_numeric",
"ocir.poor_appetite_or_overeating_base_numeric",
"ocir.feeling_bad_failure_family_base_numeric",
"ocir.trouble_concentrating_reading_newspaper_base_numeric",
"ocir.moving_fidgety_noticed_opposite_base_numeric",
"ocir.dead_hurting_thoughts_base_numeric"
)
...
dat.no.sex.no.gender.no.age.no.ibd <- dat.no.sex.no.gender.no.age.no.ibd %>% #NA per person as a percentage
  mutate(
    miss_pc_total_ocir_base = 
      na_per_person_ocir_base/9 # NA per person divided by the amount of items
  )
...
dat.no.sex.no.gender.no.age.no.ibd <- dat.no.sex.no.gender.no.age.no.ibd %>% #NA per person as a percentage
  mutate(
    miss_pc_total_ocir_retro = 
      na_per_person_ocir_retro/9 # NA per person divided by the amount of items
  )
...
dat.no.sex.no.gender.no.age.no.ibd <- dat.no.sex.no.gender.no.age.no.ibd %>% #NA per person as a percentage
  mutate(
    miss_pc_total_ocir_prepan = 
      na_per_person_ocir_prepan/9 # NA per person divided by the amount of items
  )
