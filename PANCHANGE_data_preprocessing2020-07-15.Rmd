...
```{r glad ocir cols}
glad.ocir.raw <- readRDS(file = paste0(data.raw_path, "/glad/ocir_glad.rds")) #File doesn't exist yet
dim(glad.ocir.raw)
...
glad.ocir.raw.id <- glad.ocir.raw %>%
  ...
  select(
    Sample, # Sample
    ID, # ID
    ocir.variables_original_name = ocir.variables_new_name,
    ... ##Add variables to select and rename here
  ) %>%
  mutate_if(is.numeric, ~na_if(., -88)) %>% # Recode missing values to NAs in numeric variables
  mutate_if(is.numeric, ~na_if(., -99)) %>%
  mutate_if(is.numeric, ~na_if(., -77)) %>%
...
# Create vector with items that are included in sum score
ocir.items_prepan = c(
  "ocir.all_variables_listed_here",
  ##add variables to include in sum score
  )
