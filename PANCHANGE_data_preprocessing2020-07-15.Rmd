...
glad.gad.raw.id <- glad.gad.raw.id %>%
  mutate(
    na_per_person_gad_prepan_raw =         # variable for NA per person
      rowSums(                       
...
glad.gad.raw.id %>%
  freq(na_per_person_gad_prepan_raw,
       cumul = F)
```
...
glad.gad.raw.id <- glad.gad.raw.id %>%
  mutate(
    gad.sum_score_prepan =
      if_else(condition = na_per_person_phq_prepan_raw > 0,
              true = NA_real_,
              false = gad.sum_score_prepan,
              missing = NA_real_
              )
  )

summary(glad.gad.raw.id$gad.sum_score_prepan_raw)
summary(glad.gad.raw.id$gad.sum_score_prepan)
```
