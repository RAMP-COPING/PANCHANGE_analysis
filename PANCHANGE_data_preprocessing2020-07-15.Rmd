...
glad.dem.raw.id <- glad.dem.raw %>%
  ...
  mutate(ID = as.numeric(ID),
         startDate.prepandemic = startDate) %>% # Rename startDate
  select(
         Sample, # Sample
         ID, # ID
         startDate.prepandemic, #Date when participant signed up to glad
...
if(EDGI == TRUE){
  ...
  edgi.dem.raw.id <- edgi.dem.raw %>%
    ...
    separate(externalDataReference, into = c("Sample", "ID"), sep = 4) %>% # Split ID in Sample and Number
    mutate(ID = as.numeric(ID),
           startDate.edgi = startDate) %>% # Rename startDate
    select(
      Sample, # Sample
      ID, # ID
      startDate.edgi, #Date when participant signed up to edgi
      Age_uncleaned = demographics.how_old_are_you_now.txt, # Age
...
if(NBR == TRUE){
  ...
  nbr.dem.raw.id <- nbr.dem.raw %>%
    ...
    select(
      Sample, # Sample
      ID, # ID
      startDate.coping, #Date when participant signed up to nbr
#      Age_uncleaned = demographics.how_old_are_you_now.txt, # Age
...
nbr.cohort.raw.id <- nbr.cohort.raw %>%
  ...
  mutate(
    ID = as.numeric(ID),
#         startDate.coping = startDate
         ) %>% # Rename startDate
  select(
         Sample, # Sample
         ID, # ID
         Cohort = cohort, #cohort information
#         startDate.coping, #Date when participant signed up to NBR COPING
...
ramp.dem.raw$externalDataReference <- ramp.dem.raw$`Login ID` #move Login ID to external data reference
  ...
  ramp.dem.raw.id <- ramp.dem.raw %>%
    ...
    separate(externalDataReference, into = c("Sample", "ID"), sep = 4) %>% # Split ID in Sample and Number
    mutate(ID = as.numeric(ID),
           startDate.ramp = startDate) %>% # Rename startDate
    select(
      Sample, # Sample
      ID, # ID
      startDate.ramp, #Date when participant signed up to ramp
...
ramp.employment.raw$externalDataReference <- ramp.employment.raw$`Login ID` #move Login ID to external data reference
...
ramp.employment.raw.id <- ramp.employment.raw %>%
    ...
    select(
      ...
      ID,
      startDate.ramp,
      highest_education_ramp = employment.what_is_your_highest_level_of_education, #this is missing NVQ or equivalent which we have in GLAD-COPING
      # EduYrs = employment.university_education_attend_school.txt, # Years in education - not in RAMP dataset
      employment.government_work_key_workers
    ) %>%
...
# Employment/highest education (baseline)
...
nbr.employment.raw.id <- nbr.employment.raw %>%
  ...
  #mutate(
  #  ID = as.numeric(ID) ##NBR IDs have a letter at the end so can't be coerced to numeric
  #  ) %>% 
  select(
         Sample, # Sample
         ID, # ID
...
nbr.mhd.raw.id <- nbr.mhd.raw %>%
  ...
  distinct(subjectid, .keep_all = TRUE) %>% # Remove duplicates based on ID
  separate(subjectid, into = c("Sample", "ID"), sep = 4) %>% # Split ID in Sample and Number
  mutate(#ID = as.numeric(ID), ## NBR IDs end with letters so can't be numeric
         startDate.prepandemic = startDate) %>% # Rename startDate
  select(
    Sample, # Sample
    ID, # ID
...
# Rename columns that differ from GLAD
nbr.mhd.raw.id_renamed <- nbr.mhd.raw.id %>% 
  mutate(
    mhd.specific_phobia_e.g._phobia_of_flying = mhd.specific_phobia_e.g._fear_of_flying,
    mhd.specific_phobia_e.g._phobia_of_flying_numeric = mhd.specific_phobia_e.g._fear_of_flying_numeric,
    mhd.psychological_overeating_or_bingeeating = mhd.bingeeating_disorder,
    mhd.psychological_overeating_or_bingeeating_numeric = mhd.bingeeating_disorder_numeric,
    mhd.type_psychosis_psychotic_illness = mhd.psychosis_type_psychotic_illness,
    mhd.type_psychosis_psychotic_illness_numeric = mhd.psychosis_type_psychotic_illness_numeric,
    mhd.autism_aspergers_or_autistic_spectrum_disorder = mhd.autism_spectrum_disorder_asd,
    mhd.autism_aspergers_or_autistic_spectrum_disorder_numeric = mhd.autism_spectrum_disorder_asd_numeric
    ) %>% 
  add_column(
    mhd.schizoaffective_disorder = NA, #NBR doesn't ask about schizoaffective
    mhd.schizoaffective_disorder_numeric = NA
  )

# Inspect dimensions
dim(nbr.mhd.raw.id_renamed)
# Inspect colnames
colnames(nbr.mhd.raw.id_renamed)
#Differences
dim(nbr.mhd.raw)[1]-dim(nbr.mhd.raw.id_renamed)[1]
}
```
  ...
  mutate(
    restricting_vs_binge_eating_eating_disorders_numeric =
      case_when(
        ...
        mhd.psychological_overeating_or_bingeeating_numeric == "1" ~ 2,
        (mhd.anorexia_nervosa_numeric == "0" &
        mhd.atypical_anorexia_nervosa_numeric == "0" &
        mhd.bulimia_nervosa_numeric == "0" &
        mhd.psychological_overeating_or_bingeeating_numeric == "0") ~ 0
...
[63] "mhd.schizophrenia_numeric"                                 
[64] "mhd.schizoaffective_disorder_numeric"                      
[65] "mhd.type_psychosis_psychotic_illness_numeric" 
