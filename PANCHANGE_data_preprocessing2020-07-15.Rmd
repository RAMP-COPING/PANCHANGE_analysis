...
coping.glad.gad.raw.id <- coping.glad.gad.raw.id %>%
  mutate(
...
```{r GAD base missingness 8 items}
coping.glad.gad.raw.id <- coping.glad.gad.raw.id %>%
  mutate(
    gad.missing_only_suicide_item_base = 
      if_else(
        condition = na_per_person_gad_base == 1 & 
          is.na(gad.dead_hurting_thoughts_base_numeric), 
        true = 1,
        false = 0,
        missing = NA_real_
      )
...
coping.glad.gad.raw.id %>%
  freq(gad.missing_only_suicide_item_base,
       cumul = F)
```
...
```{r GAD retro missingness 8 items}
coping.glad.gad.raw.id <- coping.glad.gad.raw.id %>%
  mutate(
    gad.missing_only_suicide_item_retro = 
      if_else(
        condition = na_per_person_gad_retro == 1 & 
          is.na(gad.dead_hurting_thoughts_retro_numeric), 
        true = 1,
        false = 0,
        missing = NA_real_
      )
...
coping.glad.gad.raw.id %>%
  freq(gad.missing_only_suicide_item_retro,
       cumul = F)
```
...
We need to identify those who have all items APART from the suicide ideation item - to still include these individuals. 
```{r PHQ base missingness 8 items}
coping.glad.phq.raw.id <- coping.glad.phq.raw.id %>%
  mutate(
...
PHQ
The PHQ sum score is set to NA if they have more than one missing item, and it is not the last suicide ideation item (they do not meet the cut off criteria - phq.missing_only_suicide_item)
```{r sum score cut offs PHQ base}
dat.no.sex.no.gender.no.age.no.ibd <- dat.no.sex.no.gender.no.age.no.ibd %>%
  mutate(
    phq.sum_score_base_full =
      if_else(na_per_person_phq_base > 1 & phq.missing_only_suicide_item == 0,
              NA_real_,
              phq.sum_score_base
              )
  )
