...
glad.ess.raw.id <- glad.ess.raw %>%
  ...
  select(
         Sample, # Sample
         ID, # ID
         ess.felt_excessively_sleepy_past= ess.day_past_month_felt,
         ess.sitting_and_reading,
         ess.watching_tv,
         ess.sitting_inactive_in_a_public_place,
         ess.break_passenger_hour_car,
...
glad.midas.raw.id <- glad.midas.raw %>%
  ...
  select(
         ...
         midas.days_past_month_miss_work_school = midas.days_past_month,
         midas.days_past_month_work_school_half_productivity = midas.days_past_month.1,
         midas.days_past_month_miss_housework = midas.days_past_month.2,
         midas.days_past_month_housework_half_productivity = midas.days_past_month.3,
         midas.days_past_month_miss_social_activities = midas.days_past_month.4
         ) %>%
  add_numeric(., exclude = exclude_cols) %>%
  mutate_if(is.numeric, ~na_if(., -88)) %>% # Recode missing values to NAs in numeric variables
  mutate_if(is.numeric, ~na_if(., -99)) %>%
...
nbr.midas.raw.id <- nbr.midas.raw %>%
  ...
  select(
         Sample, # Sample
         ID, # ID
         midas.day_past_month_felt,
         midas.sitting_and_reading,
         midas.watching_tv,
         midas.sitting_inactive_in_a_public_place,
         midas.break_passenger_hour_car,
         midas.afternoon_rest_circumstances_lying,
         midas.sitting_and_talking_to_someone,
         midas.sitting_quietly_after_a_lunch_without_alcohol,
         midas.car_minutes_stopped_traffic,
         midas.insomnia,
         midas.narcolepsy,
         midas.rem_sleep_behaviour_disorder,
         midas.restlmidas_legs_syndrome,
         midas.sleep_apnea,
         midas.sleep_terrors,
         midas.sleepwalking,
         midas.other,
...
glad.nmss.raw.id <- glad.nmss.raw %>%
  ...
  select(
         Sample, # Sample
         ID, # ID
         nmss.ability_taste_food_change,
...
edgi.nmss.raw.id <- edgi.nmss.raw %>%
  ...
  select(
         Sample, # Sample
         ID, # ID
         nmss.ability_taste_food_change = nmss.taste_food_ability_smell,
         nmss.severe_taste_smell_loss
         ) %>%
  add_numeric(., exclude = exclude_cols) %>%
  mutate_if(is.numeric, ~na_if(., -88)) %>% # Recode missing values to NAs in numeric variables
...
glad.nmsw.raw.id <- glad.nmsw.raw %>%
  ...
  select(
         ...
         nmsw.night_regularly_pass_urine,
         nmsw.sex_finding_difficult,
...
edgi.nmsw.raw.id <- edgi.nmsw.raw %>%
  ...
  select(
         ...
         ID, # ID
         nmsw.drink_problems_choking_difficulty,
         nmsw.night_regularly_pass_urine = nmsw.regularly_night_pass_urine,
         nmsw.sex_finding_difficult = nmsw.sex_difficult_finding,
         nmsw.weak_standing_feeling_light,
         nmsw.acting_moving_sleep_talking,
         nmsw.double_vision
         ) %>%
...
glad.tinnitus.raw.id <- glad.tinnitus.raw %>%
  ...
  select(
         Sample, # Sample
         ID, # ID
         tinnitus.ears_noises_head_minutes,
         tinnitus.lasts_ears_noises_head,
...
edgi.tinnitus.raw.id <- edgi.tinnitus.raw %>%
  ...
  select(
         Sample, # Sample
         ID, # ID
         tinnitus.ears_noises_head_minutes = tinnitus.ears_noises_minutes_head,
         tinnitus.lasts_ears_noises_head = tinnitus.ears_noises_minutes_head.1,
         tinnitus.noises_worry_worst_upset
         ) %>%
  add_numeric(., exclude = exclude_cols) %>%
  mutate_if(is.numeric, ~na_if(., -88)) %>% # Recode missing values to NAs in numeric variables
