...
```{r Create GLAD/EDGI prepan subset for prepan plots}

GLAD.prepan_with_EDGI <- dat.prepan %>% 
  filter(Sample == "GLAD" |
           Sample == "EDGI")
...
```{r Create NBR/EDGI labelled sample variable for retro plots}
...
dat.retro <- dat.retro %>% 
  mutate(Sample_plot = 
           case_when(
             dat.retro$Sample == "GLAD" ~ "GLAD",
             dat.retro$Sample == "EDGI" ~ "NBR/EDGI",
             dat.retro$Sample == "NBR" ~ "NBR/EDGI",
...
```{r Collapse July to September sign ups together}

dat.retro_july_sep_sign_up <- dat.retro_july_sign_up %>% 
  left_join(dat.retro_august_sign_up,
            by = "Sample_plot")

dat.retro_july_sep_sign_up <- dat.retro_july_sep_sign_up %>% 
  left_join(dat.retro_september_sign_up,
            by = "Sample_plot")

dat.retro_july_sep_sign_up <- dat.retro_july_sep_sign_up %>% 
  group_by(Sample_plot) %>% 
  mutate("July - September" = 
           July + August + September) %>% 
  select(Sample_plot, "July - September")
...
dat.retro_sign_up <- dat.retro_sign_up %>% 
  left_join(dat.retro_july_sep_sign_up,
            by = "Sample_plot")
...
dat.retro_sign_up$month <- factor(dat.retro_sign_up$month,
                                  levels = c("April", "May", "June", "July - September"))
...
GLAD.prepan_ethnicity_plot <- GLAD.prepan_with_EDGI %>% 
...
dat.retro_positive_diagnosis_by_sample <- dat.retro_positive_diagnosis_by_sample %>% 
  gather('Depressive disorders', 'Anxiety disorders', 'Eating disorders', 'OCRDs', 'Psychotic disorders', 'ASD', 'ADHD', 'Personality disorders', 'Bipolar disorders', 'PTSD', 'No diagnosis', key = "Diagnosis", value = positive_n)
...
```{r Gender age ethnicity employ retro graph with plot caption}
...
arranged_collapsed_gender_plot <- dat.retro_gender_male_female %>% 
  ...
  theme(axis.text.x = element_text(colour = "black"),
        axis.text.y = element_text(colour = "black")) +
  geom_text(x = Inf, y = Inf, label = "2", hjust = 1, vjust = 1)

```

```{r Recode ethnicity levels and re-create non-white plot}
...
```{r See non-binary/self-defined pt's by sample for caption}
...
```{r plot age gender ethnicity and employ with plot caption}
