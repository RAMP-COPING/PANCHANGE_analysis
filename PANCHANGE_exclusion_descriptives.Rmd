...
Individuals with NAs for age/gender have already been excluded in pre-processing script, but they get excluded here to be safe and to double check previous programming.
...
```{r Accuray scores for gad: worse/better than pre-pandemic}

dat.raw$gad.pandemic.change.group <- ifelse(dat.raw$gad.pandemic_felt_feelings_numeric <3,1,
                                           ifelse(dat.raw$gad.pandemic_felt_feelings_numeric > 3,2,0))

dat.raw$gad.pandemic.change.group <- factor(dat.raw$gad.pandemic.change.group,
                                           levels = c(1,0,2),labels = c("Worse","No change","Better"))

dat.raw <- dat.raw %>%
    mutate(gad.change.accurate = case_when(dat.raw$gad.pandemic.change.group == "Better" &
                                             dat.raw$gad.diff_score_base_prepan < 0 ~1,
                                           dat.raw$gad.pandemic.change.group == "Worse" &
                                             dat.raw$gad.diff_score_base_prepan > 0 ~ 1,
                                           dat.raw$gad.pandemic.change.group == "Better" &
                                             dat.raw$gad.diff_score_base_prepan > 0 ~ 0,
                                           dat.raw$gad.pandemic.change.group == "Worse" &
                                             dat.raw$gad.diff_score_base_prepan < 0 ~ 0))

dat.raw <- dat.raw %>%
    mutate(gad.change.retro.accurate = if_else(dat.raw$gad.pandemic.change.group == "Better" &
                                             dat.raw$gad.diff_score_retro_prepan == 0 |
                                             dat.raw$gad.pandemic.change.group == "Worse" &
                                             dat.raw$gad.diff_score_retro_prepan == 0, "Accurate", "Inaccurate" ))
dat.raw <- dat.raw %>%
  mutate(gad.change.retro.accurate = if_else(dat.raw$gad.pandemic.change.group == "Better" &
                                             dat.raw$gad.diff_score_retro_prepan >= -3 &
                                             dat.raw$gad.diff_score_retro_prepan <= 3 &
                                             dat.raw$gad.change.retro.accurate != "Accurate" |
                                             dat.raw$gad.pandemic.change.group == "Worse" &
                                             dat.raw$gad.diff_score_retro_prepan >= -3 &
                                             dat.raw$gad.diff_score_retro_prepan <= 3 &
                                             dat.raw$gad.change.retro.accurate != "Accurate",
                                             "Accurate +- 3", gad.change.retro.accurate))
...
```{r N completing coping per month}
...
#Correlations of retro impute and prepan
##Create function to use for all of the corplots

```{r function to create upper and lower CI matrices}
...
```{r Whole sample Correlation prep chunk}
# Select items for correlation matrix
...
```{r Whole sample correlation matrix of prepan retro sum scores}
...
```{r Whole sample prepan retro scatterplots prep}
...
whole_retro_prepan_phq_scatter <- whole_retro_prepan_scatter %>% 
  filter(phq.pandemic_felt_feelings == "Much worse" |
           ...
           phq.pandemic_felt_feelings == "Much better")


```


```{r Whole sample prepan retro scatterplots}

#Visualise as scatterplots

whole_retro_prepan_gad_scatter %>% ggplot(aes(x = gad.sum_score_prepan, y = gad.sum_score_retro)) +
  geom_point(colour = "grey") +
  geom_jitter(w = 0.4, h = 0.4, colour = "grey") +
  geom_smooth(method = "lm", colour = "navy", fill = "red") +
  theme_minimal() +
...
whole_retro_prepan_phq_scatter %>% ggplot(aes(x = phq.sum_score_retro, y = phq.sum_score_prepan)) +
  geom_point(colour = "grey") +
  geom_jitter(w = 0.4, h = 0.4, colour = "grey") +
  geom_smooth(method = "lm", colour = "navy", fill = "red") +
  theme_minimal() +
   labs(title = paste("Relationship between prepandemic and retrospective PHQ scores\nfor whole sample",
                      sep=" "),
        subtitle = paste("n", dim(whole_retro_prepan_phq_scatter)[1]))
...
```{r Little/much worse correlation prep chunk}
...
```{r Little/much worse correlation matrix of prepan retro sum scores}
...
```{r Little/much worse prepan retro scatterplot prep}
...
```{r Little/much worse prepan retro scatterplots}
...
much_little_worse_gad_scatter %>% ggplot(aes(x = gad.sum_score_prepan, y = gad.sum_score_retro)) +  
  geom_point(colour = "grey") +
  geom_jitter(w = 0.4, h = 0.4, colour = "grey") +
  geom_smooth(method = "lm", colour = "navy", fill = "red") +
  theme_minimal() +
...
```{r Little/much better correlation prep chunk}
...
```{r Little/much better correlation matrix of prepan retro sum scores}
...
```{r Little/much better prepan retro scatterplots prep}
...
```{r Little/much better prepan retro scatterplots}
...
```{r Disorder_inc by Sample prop table}
Disorder_inc_by_Sample_collapsed <- dat.long.psych %>%
  group_by(Disorder_inc) %>%
  filter(Diagnosis == 1) %>%
  count(Sample) %>%
  mutate(Prop = round(n/sum(n), 2))

kable(Disorder_inc_by_Sample_collapsed)
```
