...
dat.raw <- dat.raw %>%
    mutate(gad.change.retro.accurate = if_else(dat.raw$gad.pandemic.change.group == "Better" &
                                             ...
                                             dat.raw$gad.pandemic.change.group == "Worse" &
                                             dat.raw$gad.diff_score_retro_prepan == 0, 1, 3 ))
dat.raw <- dat.raw %>%
  mutate(gad.change.retro.accurate = if_else(dat.raw$gad.pandemic.change.group == "Better" &
                                             dat.raw$gad.diff_score_retro_prepan >= -3 &
                                             dat.raw$gad.diff_score_retro_prepan <= 3 &
                                             dat.raw$gad.change.retro.accurate_temp != 1 |
                                             dat.raw$gad.pandemic.change.group == "Worse" &
                                             dat.raw$gad.diff_score_retro_prepan >= -3 &
                                             dat.raw$gad.diff_score_retro_prepan <= 3 &
                                             dat.raw$gad.change.retro.accurate_temp != 1,  2, gad.change.retro.accurate))
...
I interpreted the presence of an NA in the retrospective score as an indicator of saying "no change" in their baseline scores. Hence, if this was the case, I imputed their baseline score. However, this interpretation could be wrong.

Also, I'm not sure what to do if there is an NA for their baseline sum score (nothing to duplicate). This was the case for ocir for some reason: the retro scores were provided for some by not the corresponding baseline score?
...
#Edit the gad sum retro score as follows: if there is a missing value in the retro sum s0core, give it the baseline sum score, else leave it as it is
...
#Edit the phq sum retro score as follows: if there is a missing value in the retro sum score, give it the baseline sum score, else leave it as it is
...
```{r Impute for ocir measure}

#Visualise the ocir summary scores for baseline and retro
head(dat$ocir.sum_score_base, 40)
head(dat$ocir.sum_score_retro, 40)

#Edit the ocir sum retro score as follows: if there is a missing value in the retro sum score, give it the baseline sum score, else leave it as it is
...
```{r Impute for pcl measure}

#Visualise the pcl summary scores for baseline and retro
head(dat$pcl.sum_score_base, 30)
head(dat$pcl.sum_score_retro, 30)

#Edit the pcl sum retro score as follows: if there is a missing value in the retro sum score, give it the baseline sum score, else leave it as it is
...
#SB - Baseline survey completion (N per month)

This is a summary of the amount of pt's completing the baseline survey per month
...
#SB - Correlation between retropsective impute and prepan

```{r Correlation prep chunk}
# Select items for correlation matrix
...
```{r Correlation matrix of prepan base sum scores}
