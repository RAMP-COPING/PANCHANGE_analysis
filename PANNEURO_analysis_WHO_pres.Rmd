...
author: "Christopher Huebel, Alish Palmos, Molly Davies, and Laura Hegemann"
date: "Sys.Date()"
output:
  html_document:
    df_print: paged
...
```{r Colour palettes: COPING, include = FALSE}
...
```{r COVID probable diagnosis frequencies}
dat.raw %>%
...
## Age
### Proportion tables
```{r age_category per Covid prop per total}
...
```{r age_category per Covid prop per Covid}
age_category_per_Covid_prop_Covid <- dat.raw %>%
  group_by(covid_probable_case_incl_screening) %>%
  count(age_category, covid_probable_case_incl_screening) %>%
  mutate(Prop = round(n/sum(n), 2)) %>% 
  drop_na()

kable(age_category_per_Covid_prop_Covid)
age_category_per_Covid_prop_Covid
```

### Cross tables
```{r age_category per Covid prop cross table}
age_category_per_Covid_prop_Covid_cross_table <- dat.raw %>%
  group_by(covid_probable_case_incl_screening) %>%
  count(age_category, covid_probable_case_incl_screening) %>%
  drop_na() %>% 
  mutate(Prop = round(n/sum(n), 2)) %>%
  pivot_wider(names_from = covid_probable_case_incl_screening, values_from = c(n, Prop))

kable(age_category_per_Covid_prop_Covid_cross_table)
age_category_per_Covid_prop_Covid_cross_table
```
...
### Proportion tables
```{r age_category_collapsed per Sample prop per total}
age_category_collapsed_per_Covid_prop_total <- dat.raw %>%
  count(age_category_collapsed, covid_probable_case_incl_screening) %>%
  mutate(Prop = round(n/sum(n), 2)) %>% 
  drop_na()

kable(age_category_collapsed_per_Covid_prop_total)
age_category_collapsed_per_Covid_prop_total
```

```{r age_category_collapsed per Sample prop per Sample}
age_category_collapsed_per_Covid_prop_Covid <- dat.raw %>%
  group_by(covid_probable_case_incl_screening) %>%
  count(age_category_collapsed, covid_probable_case_incl_screening) %>%
  mutate(Prop = round(n/sum(n), 2)) %>% 
  drop_na()

kable(age_category_collapsed_per_Covid_prop_Covid)
age_category_collapsed_per_Covid_prop_Covid
```

### Cross tables
```{r age_category_collapsed per Sample prop cross table}
age_category_collapsed_per_Covid_prop_Covid_cross_table <- dat.raw %>%
  group_by(covid_probable_case_incl_screening) %>%
  count(age_category_collapsed, covid_probable_case_incl_screening) %>%
  drop_na() %>% 
  mutate(Prop = round(n/sum(n), 2)) %>%
...
kable(age_category_collapsed_per_Covid_prop_Covid_cross_table)
age_category_collapsed_per_Covid_prop_Covid_cross_table
```

### Chi square test
```{r age_category per probable Covid chisquare test}
age_category_per_Covid_chissquare_test <- dat.raw %>%
  filter(!is.na(covid_probable_case_incl_screening)) %>%
  count(age_category_collapsed, covid_probable_case_incl_screening) %>%
  spread(key = covid_probable_case_incl_screening, value = n) %>%
  select(-age_category_collapsed) %>%
  chisq.test() %>%
  glance()

kable(age_category_per_Covid_chissquare_test)
age_category_per_Covid_chissquare_test
```
...
dat.raw$bmi[is.infinite(dat.raw$bmi)] = NA

dat.raw <- dat.raw %>%
  mutate(
  bmi.categories=cut(dat.raw$bmi, breaks=c(-Inf, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55,  Inf), labels=c("too low","10 and less","10 to 15", "15 to 20", "20 to 25", "25 to 30", "30 to 35", "35 to 40", "40 to 45", "45 to 50", "50 to 55")))

```

```{r bmi_category per Sample prop per total}

bmi_category_per_Covid_prop_total <- dat.raw %>%
  count(bmi.categories, covid_probable_case_incl_screening) %>%
  mutate(Prop = round(n/sum(n), 2))

options(knitr.kable.NA = '')
kable(bmi_category_per_Covid_prop_total)
bmi_category_per_Covid_prop_total
```
```{r bmi_category per Sample prop per Sample}
bmi_category_per_Sample_prop_Sample <- dat.raw %>%
  group_by(Sample) %>%
  count(bmi.categories, Sample) %>%
  mutate(Prop = round(n/sum(n), 2))

kable(bmi_category_per_Sample_prop_Sample)
bmi_category_per_Sample_prop_Sample
```


## Key worker status

```{r key worker status frequencies by probable Covid}
...
```{r ethnicity per probable Covid chisquare test}
ethnicity_per_Covid_chissquare_test <- dat.raw %>%
  filter(!is.na(covid_probable_case_incl_screening)) %>%
  count(Ethnicity_collapsed, covid_probable_case_incl_screening) %>%
  spread(key = covid_probable_case_incl_screening, value = n) %>%
  select(-Ethnicity_collapsed) %>%
  chisq.test() %>%
  glance()

kable(ethnicity_per_Covid_chissquare_test)
ethnicity_per_Covid_chissquare_test
```
...
# Scoring for neuro measures

# ALSfrs (not included)
```{r ALSfrs col names, include = FALSE}
...
```{r ALSfrs row sums, include = FALSE}
dat.raw <- dat.raw %>%
...
```{r ALSfrs frequencies, include = FALSE}
dat.raw %>%
  filter(!is.na(startDate.coping)) %>%
  filter(!is.na(covid_probable_case_incl_screening)) %>%
...
ess.items_key <- c(1,1,1,1,1,1,1,1)

# Create vector with items that are included in sum score
ess.items <- c(
  "ess.sitting_and_reading_numeric",
  "ess.watching_tv_numeric",
  "ess.sitting_inactive_in_a_public_place_numeric",
  "ess.break_passenger_car_hour_numeric",
  "ess.afternoon_rest_circumstances_lying_numeric",
  "ess.sitting_and_talking_to_someone_numeric",
  "ess.sitting_quietly_after_a_lunch_without_alcohol_numeric",
  "ess.car_minutes_stopped_traffic_numeric"
  )

# Calculate sum score
ess.scored_items <- scoreItems(keys = ess.items_key,
                                        items = dat.raw[ess.items],
                                        totals = TRUE, 
                                        missing = TRUE, 
                                        impute = 'none', 
                                        min = 0, 
...
dat.raw$ess.sum_score_raw <- ess.scored_items$scores

# Look at values
dat.raw %>%
  group_by(covid_probable_case_incl_screening) %>% 
  descr(ess.sum_score_raw,
        stats = "common"
        )

```

```{r ESS raw scores by Covid case}
ess_frequencies <- dat.raw %>%
  filter(!is.na(startDate.coping)) %>%
  filter(!is.na(covid_probable_case_incl_screening)) %>%
  filter(!is.na(ess.sum_score_raw)) %>%
  group_by(covid_probable_case_incl_screening) %>%
  count(covid_probable_case_incl_screening, ess.sum_score_raw) %>%
  mutate(Prop = round(n/sum(n), 2))

kable(ess_frequencies)
```

ESS scores: Interpretation
...
```{r ess clinical categories}
dat.raw <- dat.raw %>% 
  mutate(ess.cutoff.scores =
           case_when(ess.sum_score_raw <= 10 ~ "Normal sleepiness",
                     ess.sum_score_raw > 10 & ess.sum_score_raw <= 14 ~ "Mild sleepiness",
                     ess.sum_score_raw > 14 & ess.sum_score_raw <= 17 ~ "Moderate sleepiness",
                     ess.sum_score_raw > 17 & ess.sum_score_raw <= 24 ~ "Severe sleepiness",
                     )
           )
...
```{r ESS clinical categories frequencies}
ess_frequencies_clinical <- dat.raw %>%
  filter(!is.na(startDate.coping)) %>%
  filter(!is.na(covid_probable_case_incl_screening)) %>%
  filter(!is.na(ess.sum_score_raw)) %>%
  group_by(covid_probable_case_incl_screening) %>%
  count(covid_probable_case_incl_screening, ess.cutoff.scores) %>%
  mutate(Prop = round(n/sum(n), 2))

kable(ess_frequencies_clinical)
```


# NMSW
```{r nmsw col names, include = FALSE}
...
```{r select cfs variables, include = FALSE}
...
```{r calculate sum score, include = FALSE}
dat.raw <- dat.raw %>%
...
```{r get sum scores by probable Covid}
dat.raw %>%
  filter(!is.na(startDate.coping)) %>%
  filter(!is.na(covid_probable_case_incl_screening)) %>%
...
CFS cut-off scores require individual symptom-level scores to be binary.
Script below recodes variables to binary.
```{r recode cfs variables to binary, include = FALSE}
...
dat.raw$cfs.how_is_your_memory_binary <- if_else(dat.raw$cfs.how_is_your_memory_numeric >= 3, 1,0) 
...
```{r cfs select binary variables, include = FALSE}
...
```{r calculate cfs cut-off groups, include = FALSE}
dat.raw <- dat.raw %>%
...
```{r cfs fatigue cases by probable covid by Sample}
fatigue_by_covid_by_sample <- dat.raw %>%
  filter(!is.na(startDate.coping)) %>%
  filter(!is.na(covid_probable_case_incl_screening)) %>%
...
kable(fatigue_by_covid_by_sample)
```
...
dat.raw[616] <- data.frame(dat.raw[616] %>%
             map(lfactor, levels = c(0,1), labels = c("same/better", "worse")))

mem.table <- dat.raw %>%
  filter(!is.na(startDate.coping)) %>%
  filter(!is.na(covid_probable_case_incl_screening)) %>%
  filter(!is.na(cfs.how_is_your_memory)) %>%
  group_by(covid_probable_case_incl_screening) %>%
  count(covid_probable_case_incl_screening, cfs.how_is_your_memory) %>%
  mutate(Prop = round(n/sum(n), 2))
...
mem.table2 <- dat.raw %>%
  filter(!is.na(startDate.coping)) %>%
  filter(!is.na(covid_probable_case_incl_screening)) %>%
  filter(!is.na(cfs.how_is_your_memory_binary)) %>%
  group_by(covid_probable_case_incl_screening) %>%
  count(covid_probable_case_incl_screening, cfs.how_is_your_memory_binary) %>%
  mutate(Prop = round(n/sum(n), 2))
