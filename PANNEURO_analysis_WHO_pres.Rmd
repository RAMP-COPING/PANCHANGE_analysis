...
dat.raw <- dat.raw %>% 
  mutate(ess.cutoff.scores =
           case_when(ess.col.names <= 10 ~ "Normal sleepiness",
                     ess.col.names > 10 & ess.col.names <= 14 ~ "Mild sleepiness",
                     ess.col.names > 14 & ess.col.names <= 17 ~ "Moderate sleepiness",
                     ess.col.names > 17 & ess.col.names <= 24 ~ "Severe sleepiness",
                     )
           )
...
ess_frequencies <- dat.raw %>%
  filter(!is.na(startDate.coping)) %>%
  filter(!is.na(covid_probable_case_incl_screening)) %>%
  filter(!is.na(ess.cutoff.scores)) %>%
  group_by(covid_probable_case_incl_screening) %>%
