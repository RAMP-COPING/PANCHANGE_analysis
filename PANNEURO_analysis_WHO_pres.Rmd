...
## Read in data
```{r read in data}
dat.raw <- readRDS(file = paste0(data_path, "/four_cohorts.rds")) #NEEDS CHANGING
dim(dat.raw)
colnames(dat.raw)
```
...
```{r NMSW sum score frequencies}
dat.raw %>%
  filter(!is.na(startDate.coping)) %>%
  filter(!is.na(covid_test_positive)) %>%
  filter(!is.na(nmsw.sum_score)) %>%
  group_by(covid_test_positive) %>%
  count(covid_test_positive, nmsw.sum_score) %>%
  mutate(Prop = round(n/sum(n), 2))
```
