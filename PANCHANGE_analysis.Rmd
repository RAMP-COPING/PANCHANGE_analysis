...
```{r gad.sum violin plot Sample NBR EDGI combined, fig.height=14, fig.width=8, dpi=300}
dat.long.gad.sum %>%
  mutate(Time_point_renamed = fct_recode(Time_point,
            "Prepandemic" = "prepan",
            "Retrospective" = "retro",
   ...
   mutate(Sample_combined = case_when(
     Sample == "GLAD" ~ "GLAD",
     Sample == "NBR" |  Sample == "EDGI" ~ "NBR and EDGI",
     Sample == "RAMP" ~ "RAMP")
         ) %>%
  ggplot(mapping = aes(
    x = Time_point_reordered,
...
#  geom_hline(yintercept = 15, linetype = "dashed", color = "gray") +
  ...
  scale_fill_manual(
    values = c("#78D9C5","#EEB6E9"),
    breaks = c("Prepandemic", "Baseline")
  ) +
  scale_alpha(guide = 'none') +
  scale_size(guide = 'none') +
  scale_color_manual(
         ...
         tidy(get(paste0(outcome,".model1b")), conf.int = TRUE) %>%
           mutate(
             "Indepent variable" = recode_factor(
               ...
               "Gender_collapsedFemale" = "Female",
               "Gender_collapsedNon-binary/Self-defined" = "Non-binary/Self-defined",
                "age_category_collapsed16-18" = "16-18 years",
...
### Model 1b: Overlapping disorders, Adjusted
Model 0a + Gender_collapsed + age_category_collapsed + Ethnicity_collapsed + Time registration and baseline
```{r gad employment change sensitivity model1b sum_score retro glad edgi sensitivity }
# Regression coefficients
gad_prepan_model1b_education_estimates <- model1b.explan.estimates(outcome = "gad.sum_score_base",
                         explan_var = "gad.sum_score_retro",
                         data_set = "dat.prepan")
...
### Model 1b: Overlapping disorders, Adjusted
Model 0a + Gender_collapsed + age_category_collapsed + Ethnicity_collapsed + Time registration and baseline
```{r phq employment change sensitivity model1b sum_score retro in glad edgi sensitivity}
# Regression coefficients
phq_prepan_model1b_education_estimates <- model1b.explan.estimates(outcome = "phq.sum_score_base",
                         explan_var = "phq.sum_score_retro",
                         data_set = "dat.prepan")
...
```{r functions required in book for globa assignments no explan var}
lmoutput_format_noexplan <- function(lmobject,  group, dfname){
...
model1b.lm.form.noexplan <- function(outcome) {
...
```{r create formulas for forest models no explan}

model1b.lm.form.noexplan("gad.sum_score_base")

model1b.lm.form.noexplan("phq.sum_score_base")

model1b.lm.form.noexplan("pcl.sum_score_base")

```

```{r run models for all forest plot items no explan}

gad.lm.noexplan <-  lm(formula = gad.sum_score_base.glmformula,
                            dat.prepan)

phq.lm.noexplan <- lm(formula = phq.sum_score_base.glmformula,
                            dat.retro)

pcl.lm.noexplan <-  lm(formula = pcl.sum_score_base.glmformula,
                            dat.prepan)
...
dat.retro.nbr$Ethnicity_collapsed[586] <- "Other"  #this person was an NA
dat.retro.nbr$Ethnicity_collapsed[856] <- "Other"  #this person was an NA
dat.retro.nbr$Ethnicity_collapsed[989] <- "Other"  #this person was an NA

```

```{r run models for retro by sample forest plots}
...
lmoutput_format(pcl.prepan.lm,"PCL.prepandemic","PCL-6", "pcl.prepan.lm.for.plot")
lmoutput_format(pcl.retro.lm,"PCL.retrospective","PCL-6", "pcl.retro.lm.for.plot")
...
```{r generate datasets for all plots no explan}

# grouped by disorder measure
lmoutput_format_noexplan(gad.lm.noexplan,"GAD-7", "gad.lm.for.plot")
lmoutput_format_noexplan(phq.lm.noexplan,"PHQ-9", "phq.lm.for.plot")
lmoutput_format_noexplan(pcl.lm.noexplan,"PCL-6", "pcl.lm.for.plot")


# Grouped by sample
lmoutput_format_noexplan(gad.lm.noexplan,"GLAD and EDGI", "gad.lm.for.plot.GLAD")
lmoutput_format_noexplan(gad.lm.noexplan,"NBR", "gad.lm.for.plot.NBR")
lmoutput_format_noexplan(gad.lm.noexplan,"RAMP", "gad.lm.for.plot.RAMP")

lmoutput_format_noexplan(phq.lm.noexplan,"GLAD and EDGI", "phq.lm.for.plot.GLAD")
lmoutput_format_noexplan(phq.lm.noexplan,"NBR", "phq.lm.for.plot.NBR")
lmoutput_format_noexplan(phq.lm.noexplan,"RAMP", "phq.lm.for.plot.RAMP")

lmoutput_format_noexplan(pcl.lm.noexplan,"GLAD and EDGI", "pcl.lm.for.plot.GLAD")
lmoutput_format_noexplan(pcl.lm.noexplan,"NBR", "pcl.lm.for.plot.NBR")
lmoutput_format_noexplan(pcl.lm.noexplan,"RAMP", "pcl.lm.for.plot.RAMP")
...
```{r merge datasets for different combinations no explan}
# Disorder groups
plot.dat.disorder <- rbind(gad.lm.for.plot,
                                  phq.lm.for.plot,
                                  pcl.lm.for.plot)
...
plot.dat.noexplan.sample.gad <- rbind(gad.lm.for.plot.GLAD,
                             gad.lm.for.plot.NBR,
                             gad.lm.for.plot.RAMP)

plot.dat.noexplan.sample.phq <- rbind(phq.lm.for.plot.GLAD,
                             phq.lm.for.plot.NBR,
                             phq.lm.for.plot.RAMP)

plot.dat.noexplan.sample.pcl <- rbind(pcl.lm.for.plot.GLAD,
                             pcl.lm.for.plot.NBR,
                             pcl.lm.for.plot.RAMP)


plot.dat.disorder.prepan <- plot.dat.disorder.prepan %>%
  mutate(RowLabels = recode(RowLabels,
                            `PCL.prepandemic` = "PCL total prepandemic",
                            `PHQ.prepandemic` = "PHQ total prepandemic",
                            `GAD.prepandemic` = "GAD total prepandemic"))


plot.dat.disorder.retro <- plot.dat.disorder.retro %>%
  mutate(RowLabels = recode(RowLabels,
                            `PCL.retrospective` = "PCL total retrospective",
                            `PHQ.retrospective` = "PHQ total retrospective",
                            `GAD.retrospective` = "GAD total retrospective"))
...
```{r palette for forest}

forestpal <- c("#993955","#2E5077","#E6C229")

```
### disorder
title: Predictors of pandemic symptoms controlling for prepandemic symptoms
```{r forest plot by disorder prepan, fig.height=70, fig.width=55, dpi=120}

plot_text_size=60

plot_grid(
  plot_grid(
    lm.forest(plot.dat.disorder.prepan %>% filter(Categories=="Pre-existing mental health"), title="",colourpal=forestpal),
    lm.forest(plot.dat.disorder.prepan %>% filter(Categories=="Gender"), title="",colourpal=forestpal),
    lm.forest(plot.dat.disorder.prepan %>% filter(Categories=="Age"), title="",colourpal=forestpal),
    lm.forest(plot.dat.disorder.prepan %>% filter(Categories=="Ethnicity"), title="",colourpal=forestpal),
    lm.forest(plot.dat.disorder.prepan %>% filter(Categories=="Employment"), title="",colourpal=forestpal),
    lm.forest(plot.dat.disorder.prepan %>% filter(Categories=="Other"), title="",colourpal=forestpal) +
      theme(
    ...
    align = "v",
    nrow = 6,
    rel_heights = c(15/36, 2/36, 8/36, 4/36, 5/36, 2/36),
    hjust = c(0,0,0,0,0,0),
    vjust = c(2,0,0,0,0,0)
  ),
  as_ggplot(cowplot::get_legend(
    lm.forest(data %>% filter(Categories=="Other"), title="") +
      ...
      guides(color = guide_legend(order = 1, reverse = T))
  )),
  ncol= 1,
  nrow = 2,
...
```{r forest plot by disorder retro, fig.height=70, fig.width=55, dpi=120}

plot_grid(
  plot_grid(
    lm.forest(plot.dat.disorder.retro %>% filter(Categories=="Preexisting mental health"), title="",colourpal=forestpal),
    lm.forest(plot.dat.disorder.retro %>% filter(Categories=="Gender"), title="",colourpal=forestpal),
    lm.forest(plot.dat.disorder.retro %>% filter(Categories=="Age"), title="",colourpal=forestpal),
    lm.forest(plot.dat.disorder.retro %>% filter(Categories=="Ethnicity"), title="",colourpal=forestpal),
    lm.forest(plot.dat.disorder.retro %>% filter(Categories=="Employment"), title="",colourpal=forestpal),
    lm.forest(plot.dat.disorder.retro %>% filter(Categories=="Other"), title="",colourpal=forestpal) +
      theme(
        axis.title.x = element_text(size = plot_text_size),
        axis.text.x = element_text(size = plot_text_size)),
...
```{r forest plot by disorder no explan, fig.height=70, fig.width=55, dpi=120}

#lm.forest(plot.dat.disorder,"Predictors of pandemic symptoms")

```

### disorder (no PCL)
```{r forest plot by disorder prepan no pcl, fig.height=70, fig.width=55, dpi=120}

#lm.forest(plot.dat.disorder.prepan.gadphq,"Predictors of pandemic symptoms controlling for prepandemic symptoms")

```

```{r forest plot by disorder retro no pcl, fig.height=70, fig.width=55, dpi=120}
...
### time point 
```{r forest plot by time point gad, fig.height=70, fig.width=55, dpi=120}

#lm.forest(plot.dat.time.gad,"Predictors of anxiety during pandemic controlling for retrospective or prepandemic symptoms")

```

```{r forest plot by time point phq, fig.height=70, fig.width=55, dpi=120}
...
```{r forest plot by time point pcl, fig.height=70, fig.width=55, dpi=120}
...
```{r forest plot by sample retro only gad, fig.height=70, fig.width=55, dpi=120}

#lm.forest(plot.dat.sample.gad ,"Predictors of anxiety controlling for retrospective symptoms across samples")

```

```{r forest plot by sample retro only phq, fig.height=70, fig.width=55, dpi=120}

#lm.forest(plot.dat.sample.phq ,"Predictors of depression controlling for retrospective symptoms across samples")

```

### Sample pandemic only, no explanatory variable)

```{r forest plot by sample no explan gad, fig.height=70, fig.width=55, dpi=120}

#lm.forest(plot.dat.noexplan.sample.gad  ,"Predictors of anxiety across samples")

```


```{r forest plot by sample no explan phq, fig.height=70, fig.width=55, dpi=120}

#lm.forest(plot.dat.noexplan.sample.phq  ,"Predictors of depression across samples")

```


```{r forest plot by sample no explan pcl, fig.height=70, fig.width=55, dpi=120}

#lm.forest(plot.dat.noexplan.sample.pcl  ,"Predictors of symptoms of PTSD across samples")

```

# Retrospective regression models by sample

## GAD

### GLAD
```{r gad employment prior covid sensitivity model1b sum_score retro by sample glad}
# Regression coefficients
gad_retro_model1b_explan_estimates <- model1b.explan.estimates(outcome = "gad.sum_score_base",
                                                               explan_var = "gad.sum_score_retro",
                                                               data_set = "dat.retro.glad")
kable(gad_retro_model1b_explan_estimates)
gad_retro_model1b_explan_estimates
...
### NBR
```{r gad employment prior covid sensitivity model1b sum_score retro by sample nbr}
# Regression coefficients
gad_retro_model1b_explan_estimates <- model1b.explan.estimates(outcome = "gad.sum_score_base",
                                                               explan_var = "gad.sum_score_retro",
                                                               data_set = "dat.retro.nbr")
kable(gad_retro_model1b_explan_estimates)
gad_retro_model1b_explan_estimates
...
### RAMP
```{r gad employment prior covid sensitivity model1b sum_score retro by sample ramp}
# Regression coefficients
gad_retro_model1b_explan_estimates <- model1b.explan.estimates(outcome = "gad.sum_score_base",
                                                               explan_var = "gad.sum_score_retro",
                                                               data_set = "dat.retro.ramp")
kable(gad_retro_model1b_explan_estimates)
gad_retro_model1b_explan_estimates
...
### GLAD
```{r phq employment prior covid sensitivity model1b sum_score retro by sample glad}
# Regression coefficients
phq_retro_model1b_explan_estimates <- model1b.explan.estimates(outcome = "phq.sum_score_base",
                                                               explan_var = "phq.sum_score_retro",
                                                               data_set = "dat.retro.glad")
...
### NBR
```{r phq employment prior covid sensitivity model1b sum_score retro by sample nbr}
# Regression coefficients
phq_retro_model1b_explan_estimates <- model1b.explan.estimates(outcome = "phq.sum_score_base",
                                                               explan_var = "phq.sum_score_retro",
                                                               data_set = "dat.retro.nbr")
...
### RAMP
```{r phq employment prior covid sensitivity model1b sum_score retro by sample ramp}
# Regression coefficients
phq_retro_model1b_explan_estimates <- model1b.explan.estimates(outcome = "phq.sum_score_base",
                                                               explan_var = "phq.sum_score_retro",
                                                               data_set = "dat.retro.ramp")
...
### GLAD
```{r pcl employment prior covid sensitivity model1b sum_score retro by sample glad}
# Regression coefficients
pcl_retro_model1b_explan_estimates <- model1b.explan.estimates(outcome = "pcl.sum_score_base",
                                                               explan_var = "pcl.sum_score_retro",
                                                               data_set = "dat.retro.glad")
kable(pcl_retro_model1b_explan_estimates)
pcl_retro_model1b_explan_estimates


```

### NBR
```{r pcl employment prior covid sensitivity model1b sum_score retro by sample nbr}
# Regression coefficients
pcl_retro_model1b_explan_estimates <- model1b.explan.estimates(outcome = "pcl.sum_score_base",
                                                               explan_var = "pcl.sum_score_retro",
                                                               data_set = "dat.retro.nbr")
...
### RAMP
```{r pcl employment prior covid sensitivity model1b sum_score retro by sample ramp}
# Regression coefficients
pcl_retro_model1b_explan_estimates <- model1b.explan.estimates(outcome = "pcl.sum_score_base",
                                                               explan_var = "pcl.sum_score_retro",
                                                               data_set = "dat.retro.ramp")
...
# frequencies and descriptives in exclusion data

after excluding for age, gender, or MHD being NA (as in skipped or missed. Prefer not to answer was categories as it's own option), and if they did not complete all measurement points.


### sample size counts by cohort for each measurement point

```{r freq table overall by sample}
...
```{r freq table prepandemic by sample}
...
```{r freq table retrospective by sample}
...
```{r freq table gad perceived change}
dat %>%
  group_by(Sample) %>%
...
```{r freq table phq perceived change}
...
```{r freq table employment collapsed}
dat %>%
  group_by(Sample) %>%
...
### anxiety disorders
```{r freq table diagnosis anxiety}
dat %>%
  group_by(Sample) %>%
...
### Depressive disorders
```{r freq table diagnosis depression}
dat %>%
  group_by(Sample) %>%
  freq(depressive_disorders) 
```
### Depression and anxiety
```{r freq table diagnosis depression and anxiety}
dat %>%
  group_by(Sample) %>%
  freq(depression_and_anxiety) 
```
### Eating disorders
```{r freq table diagnosis eating disorder}
dat %>%
  group_by(Sample) %>%
...
```
### OCD
```{r freq table diagnosis obsessive compulsive disorders}
dat %>%
  group_by(Sample) %>%
...
### Psychosis
```{r freq table diagnosis psychotic disorders}
dat %>%
  group_by(Sample) %>%
...
### Bipolar
```{r freq table diagnosis bipolar}
dat %>%
  group_by(Sample) %>%
...
### Personality disorders
```{r freq table diagnosis personality disorder}
dat %>%
  group_by(Sample) %>%
...
### Controls 
```{r freq table diagnosis control}
dat %>%
  group_by(Sample) %>%
  freq(control) 
```
...
```{r freq table diagnosis prefer not to say}
dat %>%
  group_by(Sample) %>%
...
retrospective OPHQ 9 vs baseline.

Notes: We use the default (Welch's) t-test (to account for non equal variance)
```{r t tests }

datgladedgi <- dat.retro %>%
  filter(Sample == "GLAD") 

t.test(datgladedgi$phq.sum_score_base,
       datgladedgi$phq.sum_score_retro,
       paired=T)

t.test(datgladedgi$gad.sum_score_base,
       datgladedgi$gad.sum_score_retro,
       paired=T)

```

## Sensitivity: retrospective regression (excluding NBR)

retrospective dataset, dropping NBR (which includes edgi)
```{r make no nbr dat}

dat.retro.noNBR <- dat.retro %>%
  filter(Sample != "NBR" & 
           Sample != "EDGI")
...
#### sensitivity
```{r retro sensitivty no NBR gad}

gad_retro_model1b_explan_estimates <- model1b.explan.estimates(outcome = "gad.sum_score_base",
                                                               explan_var = "gad.sum_score_retro",
                                                               data_set = "dat.retro.noNBR")
kable(gad_retro_model1b_explan_estimates)
gad_retro_model1b_explan_estimates
