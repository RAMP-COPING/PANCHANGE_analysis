...
++MD: I had to cancel this chunk while running because it took too long - there are 473 variables in this dataset, so I don't think summarising all at once is useful!
...
# Exclusion
Filter data frame to create 3 datasets:
  1. Diagnoses analyses
        i. Exclude individuals with NA for MHD
  2. Retrospective - baseline change anlyses
        i. Exclude individuals with NAs for:
              1. MHD
              2. Age
              3. Gender
  3. Prepandemic - baseline change analyses
          i. Exclude individuals with NAs for:
              1. MHD
              2. Age
              3. Gender
...
Diagnoses analyses
```{r dat.mhd - Vectors with sum_score_cols}
sum_score_cols <- dat.mhd %>%
...
diff_score_cols <- dat.mhd %>%
  select(contains("diff_score")) %>% 
  colnames()

diff_score_cols

phq.diff_score_cols <- dat.mhd %>%
  select(contains("phq")) %>%
  select(contains("diff_score")) %>% 
  colnames()
...
## Hierarchy including psychosis
Diagnoses analyses
```{r Psychiatric disorders hierarchy including psychosis - dat.mhd}
dat.mhd <- dat.mhd %>%
  mutate(
...
Retro-baseline
```{r Psychiatric disorders hierarchy including psychosis - dat.retro}
dat.retro <- dat.retro %>%
  mutate(
    Disorder_hierarchical_psychosis_numeric =
      case_when(
...
Prepan-baseline
++MD: For GLAD/EDGI we shouldn't have any controls, but some have crept in because they likely haven't ever received a diagnosis (despite qualifying on the CIDI or current measures of psychopathology). We should consider just excluding them.
```{r Psychiatric disorders hierarchy including psychosis - dat.prepan}
...
## Hierarchy NOT including psychosis
Diagnoses analyses
```{r Psychiatric disorders hierarchy without psychosis - dat.mhd}
dat.mhd <- dat.mhd %>%
  mutate(
...
Retro-baseline
```{r Psychiatric disorders hierarchy without psychosis - dat.retro}
dat.retro <- dat.retro %>%
  mutate(
    Disorder_hierarchical_numeric =
      case_when(
...
Prepan-baseline
```{r Psychiatric disorders hierarchy without psychosis - dat.prepan}
dat.prepan <- dat.prepan %>%
  mutate(
    Disorder_hierarchical_numeric =
      case_when(
...
Investigate NAs 
Ask Leo?
++MD: you'll have NAs for individuals that don't have one of the listed disorders in the hierarchy, but who have another diagnosis that excludes them from being a control (e.g. personality disorder, ADHD, ASD). We can add an "Other" category to cover those other diagnoses, or exclude them from these analyses.
```{r NAs in diagnosis}
dat.mhd %>%
...
## Correlation matrix
++MD: Should we use the dat.mhd for this correlation matrix, as it has the least stringent inclusion criteria? Or just use dat.raw and include everyone? (alternatively, can create a correlation matrix for all 3 datasets)
...
mhd.corr.mat <- hetcor(
  as.data.frame(dat.mhd[,mhd.items]), # Specify the data
