...
Disorder_by_Ethnicity <- dat.long.psych %>%
  filter(Diagnosis == 1) %>%
  count(Disorder, Ethnicity) %>%
  mutate(Prop = round(n/sum(n), 2))

Disorder_by_Ethnicity
```

```{r Disorder by Ethnicity plot}
Disorder_by_Ethnicity %>%
  ggplot(mapping = aes(
    x = Disorder,
    y = n)
...
```{r Gender per Sample chisquare}
dat %>% 
  specify(Gender ~ Sample)  %>% 
  hypothesize(null = "independence") %>% 
  generate(reps = 1000, type = "permute") %>% 
  calculate(stat = "Chisq") %>% 
  visualize()# +
#  geom_vline(aes(xintercept = observed_stat), color = "red") # add a vertical line for the data
```
