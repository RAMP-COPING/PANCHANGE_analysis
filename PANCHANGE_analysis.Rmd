...
```{r Gender per Sample count cross table chi square test}
dat %>%
  count(age_category, Sample) %>%
  spread(key = Sample, value = n) %>%
  select(-age_category) %>%
...
Correlation matrix of psychiatric disorders
...
# Ethnicity
```{r Ethnicity per Sample prop per total}
Ethnicity_by_Sample_prop_total <- dat %>%
  count(Ethnicity, Sample) %>%
  mutate(Prop = round(n/sum(n), 2))

Ethnicity_by_Sample_prop_total
```


```{r Ethnicity per Sample prop per sample}
Ethnicity_by_Sample_prop_sample <- dat %>%
  group_by(Sample) %>%
  count(Ethnicity, Sample) %>%
  mutate(Prop = round(n/sum(n), 2))

Ethnicity_by_Sample_prop_sample
```

```{r Ethnicity per Sample prop cross table}
dat %>%
  group_by(Sample) %>%
  count(Ethnicity, Sample) %>%
  mutate(Prop = round(n/sum(n), 2)) %>%
...
```{r Ethnicity per Sample count cross table}
dat %>%
  count(Ethnicity, Sample) %>%
  spread(key = Sample, value = n) %>%
  mutate(
...
```{r Ethnicity per Sample chisquare test}
dat %>%
  count(Ethnicity, Sample) %>%
  spread(key = Sample, value = n) %>%
  select(-Ethnicity) %>%
...
Gender_per_Sample_prop_total <- dat %>%
  count(Gender, Sample) %>%
  mutate(Prop = round(n/sum(n), 2))

kable(Gender_per_Sample_prop_total)
Gender_per_Sample_prop_total
```

```{r Gender per Sample prop per Sample}
Gender_per_Sample_prop_Sample <- dat %>%
  group_by(Sample) %>%
  count(Gender, Sample) %>%
  mutate(Prop = round(n/sum(n), 2))

kable(Gender_per_Sample_prop_Sample)
Gender_per_Sample_prop_Sample
```

```{r Gender per Sample prop cross table}
Gender_per_Sample_prop_Sample_cross_table <- dat %>%
  group_by(Sample) %>%
  count(Gender, Sample) %>%
  mutate(Prop = round(n/sum(n), 2)) %>%
  pivot_wider(names_from = Sample, values_from = c(n, Prop))

kable(Gender_per_Sample_prop_Sample_cross_table)
Gender_per_Sample_prop_Sample_cross_table
```


```{r Gender per Sample count cross table}
Gender_per_Sample_count_cross_table <- dat %>%
  count(Gender, Sample) %>%
  spread(key = Sample, value = n) %>%
  mutate(
    EDGI = as.integer(replace_na(EDGI, 0)),
...
kable(Gender_per_Sample_count_cross_table)
Gender_per_Sample_count_cross_table
```


```{r Gender per Sample chisquare test}
Gender_per_Sample_chissquare_test <- dat %>%
  count(Gender, Sample) %>%
  spread(key = Sample, value = n) %>%
  select(-Gender) %>%
  mutate(
...
kable(Gender_per_Sample_chissquare_test)
Gender_per_Sample_chissquare_test
```

```{r Gender by Sample count plot}
Gender_by_Sample_count_plot <- Gender_per_Sample_prop_Sample %>%
  ggplot(mapping = aes(
    x = Gender,
    y = n)
  ) +
...
Gender_by_Sample_count_plot
```

```{r Gender by Sample prop plot}
Gender_by_Sample_prop_plot <- Gender_per_Sample_prop_Sample %>%
  ggplot(mapping = aes(
    x = Gender,
    y = Prop)
  ) +
...
model1 <- lm(
  formula = phq.diff_score_base_prepan ~
    ...
    mhd.attention_deficit_hyperactivity_disorder +
    mhd.personality_disorder +
    Gender + age_category + Ethnicity + time_diff_sign_up_coping,
  data = dat)
...
```{r}
model1 <- lm(
  formula = pcl.diff_score_base_prepan ~ Disorder_hierarchical + Gender + age_category + Ethnicity + time_diff_sign_up_coping,
  data = dat)
